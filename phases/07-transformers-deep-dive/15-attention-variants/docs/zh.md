# 注意力机制 Variants — Sliding Window, Sparse, Differential
...
**类型:** 实现
**Languages:** Python
**Prerequisites:** Phase 7 · 02 (Self-注意力机制), Phase 7 · 03 (Multi-Head), Phase 7 · 12 (KV Cache / Flash 注意力机制)
**Time:** ~60 分钟

## The Problem

Full attention costs `O(N²)` memory and `O(N²)` compute in sequence length. 对于 a 128K-context Llama 3 70B that is 16 billion attention entries per layer, times 80 layers. Flash 注意力机制 (Lesson 12) hides the `O(N²)` activation memory but does not change the arithmetic cost — every token still attends to every other token.
...
对于 `N = 8192` and `W = 1024`, the score matrix has 1024 × 8192 non-zero rows in expectation — an 8× reduction.

**KV cache shrinks with SWA.** Only the last `W` tokens of K and V need to be kept per layer. 对于 a Gemma-3-ish config (1024 window, 128K context), KV cache drops 128×.
...
- **Native Sparse 注意力机制 (DeepSeek, 2025).** Learn which blocks of `(Q, K)` matter; skip the zero blocks at kernel level. FlashAttention-compatible.
...
### Differential 注意力机制 (DIFF Transformer, 2024)
...
One parameter — `window`. 对于 `window >= n`, you recover full causal attention. 对于 `window = 1`, each token attends only to itself.
...
This compiles to a custom Triton kernel. 使用in 10% of FlashAttention-3 speed for common patterns, and the mask function is a Python callable.
...
| Native Sparse 注意力机制 | "DeepSeek's kernel trick" | Learn block-level sparsity; skip zero blocks at the kernel level while keeping quality. |
...
| 注意力机制 sink | "Weight bleeds to token 0" | Softmax normalization forces rows to sum to 1; uninformative queries dump weight on position 0. |
...
- [Yuan et al. (2025). Native Sparse 注意力机制](https://arxiv.org/abs/2502.11089) — DeepSeek-V3.2's learned-sparsity attention.
