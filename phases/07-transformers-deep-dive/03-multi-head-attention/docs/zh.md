...
Multi-head attention is the default every transformer in 2026 ships with. The only argument is about *how many* heads and whether keys and values share projections (Grouped-查询 注意力机制, Multi-查询 注意力机制, Multi-head Latent 注意力机制).
...
Each head gets its own slice of Q, K, V. 注意力机制 becomes a batched matmul:
...
### Step 3: Grouped-查询 注意力机制 variant
...
Run MHA on a short sentence with 4 heads. 对于 each head, print the `(N, N)` attention matrix. You'll see different heads pick out different structure even with random initialization — that's partly signal, partly rotational symmetry in the subspaces.
...
3. **Hard.** Implement a tiny version of Multi-head Latent 注意力机制: compress K,V to a rank-`r` latent, store the latent in the KV cache, decompress at attention time. At what `r` does cache memory cross below 1/8 of full MHA while quality stays within 1 bit of validation ppl?
...
| MQA | "One KV head" | Multi-查询 注意力机制: single shared K/V projection. Smallest KV cache, some quality loss. |
| GQA | "The default since Llama 2" | Grouped-查询 注意力机制 with `n_kv_heads < n_heads`; repeats to match Q. |
| MLA | "DeepSeek's trick" | Multi-head Latent 注意力机制: K,V compressed to low-rank latent, decompressed at attend time. |
...
- [Vaswani et al. (2017). 注意力机制 Is All You Need §3.2.2](https://arxiv.org/abs/1706.03762) — the original multi-head spec.
- [Shazeer (2019). Fast Transformer Decoding: One Write-Head is All You Need](https://arxiv.org/abs/1911.02150) — the MQA paper.
- [Ainslie et al. (2023). GQA: Training Generalized Multi-查询 Transformer Models from Multi-Head Checkpoints](https://arxiv.org/abs/2305.13245) — how to convert MHA to GQA after training.
