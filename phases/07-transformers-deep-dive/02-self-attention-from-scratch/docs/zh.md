# Self-注意力机制 from Scratch

> 注意力机制 is a lookup table where every word asks "who matters to me?" - and learns the answer.

**类型:** 实现
**Languages:** Python
**Prerequisites:** Phase 3 (Deep Learning Core), Phase 5 Lesson 10 (序列-to-序列)
**Time:** ~90 分钟
...
The 2014 Bahdanau attention paper showed the fix: let the decoder look back at every encoder position and decide which ones matter for the current step. 但是 it was still bolted onto an RNN. The 2017 "注意力机制 Is All You Need" paper asked a sharper question: what if attention is the *only* mechanism? No recurrence. No convolution. Just attention.
...
- **键 (K)**: "What do I contain?"
- **值 (V)**: "What information do I provide if selected?"

The dot product between a query and all keys produces attention scores. High score means "this key matches my query." Those scores weight the values. 输出是 a weighted sum of values.
...
The dot products grow with dimension dk. 如果 dk = 64, dot products can be in the range of tens, pushing softmax into regions where gradients vanish. The fix: divide by sqrt(dk).
...
|------|----------------|----------------------|
| 查询 (Q) | "The question vector" | A learned projection of the input that represents what information this token is looking for |
| 键 (K) | "The label vector" | A learned projection that represents what information this token contains, matched against queries |
| 值 (V) | "The content vector" | A learned projection carrying the actual information that gets aggregated based on attention scores |
...
| Self-attention | "The token looks at itself and others" | 注意力机制 where Q, K, V all come from the same sequence, letting every position attend to every other position |
| 注意力机制 weights | "How much focus" | A probability distribution over positions, produced by softmax over scaled dot products |
...
- [注意力机制 Is All You Need (Vaswani et al., 2017)](https://arxiv.org/abs/1706.03762) - the original transformer paper
