...
> 注意力机制 is the star. Everything else — residuals, normalization, feed-forward, cross-attention — is the scaffolding that lets you stack it deep.

**类型:** 实现
**Languages:** Python
**Prerequisites:** Phase 7 · 02 (Self-注意力机制), Phase 7 · 03 (Multi-Head 注意力机制), Phase 7 · 04 (位置编码)
**Time:** ~75 分钟
...
![编码器 and decoder block internals, wired](../assets/full-transformer.svg)

### The six pieces

1. **嵌入 + positional signal.** Tokens → vectors. Position injected via RoPE (modern) or sinusoidal (classic).
...
4. **Residual connection.** `x + sublayer(x)`. 使用out this, gradients vanish past ~6 layers.
...
### 编码器 block (used by BERT, T5 encoder)
...
编码器 is bidirectional. No masking. All positions see all positions.

### 解码器 block (used by GPT, T5 decoder)

```
x → LN → MHA(masked self) → + → LN → MHA(cross to encoder) → + → LN → FFN → + → out
```

解码器 has three sublayers per block. The middle one — cross-attention — is the only place information flows from encoder to decoder. In a pure decoder-only architecture (GPT), cross-attention is omitted and you just have masked self-attention + FFN.

### Pre-norm vs post-norm

或者iginal paper: `x + sublayer(LN(x))` vs `LN(x + sublayer(x))`. Post-norm lost favor around 2019 — it is harder to train deeply without careful warmup. Pre-norm (`LN` *before* sublayer) is the 2026 default: Llama, Qwen, GPT-3+, Mistral all use it.
...
对于 one block with `d_model = d` and FFN expansion `r`:
...
使用 the tiny `Matrix` class from Lesson 03 (copied to this file for independence):
...
The PyTorch/TF reference implementations: `nn.TransformerEncoderLayer`, `nn.TransformerDecoderLayer`. 但是 most 2026 production code rolls its own block because:

- Flash 注意力机制 is called inside attention, not via `nn.MultiheadAttention`.
...
**编码器 vs decoder vs encoder-decoder — when to pick:**

| Need | Pick | Example |
|------|------|---------|
| Classification, embeddings, QA over text | 编码器-only | BERT, DeBERTa, ModernBERT |
| Text generation, chat, code, reasoning | 解码器-only | GPT, Llama, Claude, Qwen |
| Structured input → structured output (translation, summarization) | 编码器-decoder | T5, BART, Whisper |

解码器-only won language because it scales cleanest and handles both comprehension and generation. 编码器-decoder is still best when the input has a clear "source sequence" identity (translation, speech recognition, structured tasks).
...
- [Vaswani et al. (2017). 注意力机制 Is All You Need](https://arxiv.org/abs/1706.03762) — original block spec.
