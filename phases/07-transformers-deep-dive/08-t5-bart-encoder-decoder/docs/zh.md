...
解码器-only GPT and encoder-only BERT each strip down the 2017 architecture for a different goal. 但是 many tasks are naturally input-output:
...
对于 these, encoder-decoder makes the cleanest fit. The encoder produces a dense representation of the source. The decoder generates the output, cross-attending to that representation at every step. Training is shift-by-one on the output side. Same loss as GPT, just conditioned on the encoder output.
...
解码器-only won the spotlight, but encoder-decoder never went away.

## The Concept

![编码器-decoder with cross-attention](../assets/encoder-decoder.svg)
...
### 当 to pick each variant in 2026

| Task | 编码器-decoder? | Why |
|------|------------------|-----|
...
| Code completion | Usually no | 解码器-only with long context wins; code models like Qwen 2.5 Coder are decoder-only |
...
Given the corrupted input and target, reconstruct the original sentence. 如果 your corruption is reversible, the forward pass is well-defined. This is a sanity check — real training never does this, but the test is cheap and catches off-by-one bugs in your span bookkeeping.
...
|------|-----------------|-----------------------|
| 编码器-decoder | "Seq2seq transformer" | Two stacks: bidirectional encoder for input, causal decoder with cross-attention for output. |
| Cross-attention | "Where source talks to target" | 解码器's Q × encoder's K/V. The only place encoder information enters the decoder. |
...
- [Lewis et al. (2019). BART: Denoising 序列-to-序列 Pre-training for Natural Language Generation, Translation, and Comprehension](https://arxiv.org/abs/1910.13461) — BART.
