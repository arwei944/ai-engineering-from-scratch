...
**Type:** Learn
**Languages:** Python
**Prerequisites:** Phase 3 (Deep Learning Core), Phase 5 · 09 (序列-to-序列), Phase 5 · 10 (注意力机制 Mechanism)
**Time:** ~45 分钟
...
The 2017 paper "注意力机制 Is All You Need" proposed something radical: drop recurrence entirely. Let every position attend to every other position in parallel. Train in one big matrix multiplication instead of 1,024 sequential ones.
...
**注意力机制 as a broadcast.** Self-attention computes `output_i = sum_j(a_ij * v_j)` for every pair `(i, j)` simultaneously. The whole N×N attention matrix fills in one batched matmul. No step depends on another. GPUs love it.

**The speedup is not a constant.** It is the difference between `O(N)` serial depth and `O(1)` serial depth. In practice, transformers train 5–10× faster per epoch on matched hardware at N=512, and the gap widens with sequence length until you hit the `O(N²)` memory wall of attention (which Flash 注意力机制 later fixed — see Lesson 12).

**What transformers cost.** 注意力机制 memory scales as `O(N²)`. 对于 2K context, fine. 对于 128K context, you need sliding windows, RoPE extrapolation, Flash 注意力机制 tiling, or linear attention variants. Recurrence was `O(N)` in both time and memory; transformers trade time for memory and then win the time back through parallelism.
...
Both algorithms do N adds. 区别是 *dependency depth*: how many operations must happen sequentially before the next can start. RNN depth = N. 注意力机制 depth = log(N) with a tree reduction, or 1 with a parallel scan. Depth, not op count, decides GPU time.
...
当 to still pick an RNN in 2026:
...
| 注意力机制 | "Let tokens look at each other" | Weighted sum `sum_j a_ij v_j` where `a_ij` comes from a similarity score between positions i and j. |
...
| Quadratic bottleneck | "Why context costs so much" | 注意力机制 memory = `O(N²)` in sequence length; Flash 注意力机制 hides the constants, not the scaling. |

## 延伸阅读

- [Vaswani et al. (2017). 注意力机制 Is All You Need](https://arxiv.org/abs/1706.03762) — the paper that killed recurrence in mainstream NLP.
...
- [Gu, Dao (2023). Mamba: Linear-Time 序列 Modeling with Selective State Spaces](https://arxiv.org/abs/2312.00752) — modern recurrent answer to transformers.
