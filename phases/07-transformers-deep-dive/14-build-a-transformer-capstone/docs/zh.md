...
The capstone: train a small decoder-only transformer end-to-end on a character-level language modeling task. It reads Shakespeare. It generates new Shakespeare. It is small enough to train on a laptop in under 10 分钟. It is correct enough that swapping in a bigger dataset and longer training gets you a real LM.
...
- Flash 注意力机制 — PyTorch 2.0+ auto-dispatches if the inputs match; we use `F.scaled_dot_product_attention`.
...
- Training loss converges from ~4.2 (random) to ~1.5 in about 6 分钟.
...
Get a random batch of length-256 token windows. 对于ward. Shift-by-one cross-entropy. Backward. AdamW step. Log. Repeat.
...
Not Shakespeare. 但是 Shakespeare-shaped. A clear win for ~800K parameters and 6 分钟 on a laptop.
...
2. **Train on a bigger corpus.** Use `OpenWebText` or `fineweb-edu` (HuggingFace). 10B tokens on a single A100 takes ~24 小时 for a 125M-param GPT.
3. **Add RoPE + KV cache + Flash 注意力机制.** The exercises below walk you through each.

This ends up as a 125M-parameter GPT that generates fluent English. Not a frontier model. 但是 the same code path — just bigger — is what Karpathy, EleutherAI, and the Allen Institute use to train research checkpoints in 2026.
...
4. **Hard.** Add a second head to the model that predicts the next-plus-one token (MTP — Multi-词元 Prediction from DeepSeek-V3). Train jointly. Does it help?
