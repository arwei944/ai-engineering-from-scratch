...
Stack L blocks of `LayerNorm → Self-注意力机制 → + → LayerNorm → MLP → +`. Identical to BERT. No vision-specific layers. This is the pedagogical punchline of the paper.

### Step 5 — head

对于 classification: take `[CLS]` hidden state → linear → softmax. 对于 DINOv2 or SAM, discard `[CLS]`, use the patch embeddings directly.
...
ViT needs *a lot* of data to match CNNs because it has none of the CNN inductive biases (translation invariance, locality). 使用out >100M labeled images or strong self-supervised pretraining, CNNs still win at matched compute. DeiT fixed this in 2021 with distillation tricks; DINOv2 fixed it permanently in 2023 with self-supervision.

## 从零实现

See `code/main.py`. Pure-stdlib patchify + linear embedding + sanity checks. No training — ViT at any realistic scale needs PyTorch and 小时 of GPU time.
...
- [Liu et al. (2021). Swin Transformer: Hierarchical 视觉Transformer using Shifted Windows](https://arxiv.org/abs/2103.14030) — Swin.
