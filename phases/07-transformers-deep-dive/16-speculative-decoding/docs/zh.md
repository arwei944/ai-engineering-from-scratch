# 推测解码 — Draft, Verify, Repeat

> Autoregressive decoding is serial. Each token waits for the previous one. Speculative decoding breaks the chain: a cheap model drafts N tokens, the expensive model verifies all N in one forward pass. 当 the draft is right you paid one big forward for N generations.

**类型:** 实现
**Languages:** Python
**Prerequisites:** Phase 7 · 07 (GPT Causal LM), Phase 7 · 12 (KV Cache & Flash 注意力机制)
**Time:** ~60 分钟

## The Problem

A 70B LLM sampling one token takes ~30 ms on an H100. A 3B draft model takes ~3 ms. 如果 we let the 3B draft 5 tokens ahead, then run the 70B *once* to verify all 5, the total is `5×3 + 30 = 45 ms` for up to 5 accepted tokens — versus `5×30 = 150 ms` for straight-line generation. That is the full speculative-decoding pitch: trade a small amount of extra GPU memory (draft model) for 2–4× lower decode latency.
...
Verification feeds `N` draft tokens into the verifier in one forward pass. This extends the verifier's KV cache by `N` entries. 如果 some drafts are rejected, you must roll the cache back to the accepted prefix length.
...
| Bonus token | "The free one" | 当 all N drafts accepted, sample one more from the verifier's next-step distribution. |
...
- [Leviathan, Kalman, Matias (2023). Fast Inference from Transformers via 推测解码](https://arxiv.org/abs/2211.17192) — the core algorithm and the equivalence theorem.
...
- [Fu et al. (2024). Break the Sequential Dependency of LLM Inference 使用 Lookahead Decoding](https://arxiv.org/abs/2402.02057) — lookahead, no-draft approach.
- [vLLM docs — 推测解码](https://docs.vllm.ai/en/latest/features/spec_decode.html) — canonical production reference with all four strategies wired up.
