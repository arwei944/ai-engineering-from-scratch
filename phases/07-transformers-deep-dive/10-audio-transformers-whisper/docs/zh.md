...
**Type:** Learn
**Languages:** Python
**Prerequisites:** Phase 7 · 05 (Full Transformer), Phase 7 · 08 (编码器-解码器), Phase 7 · 09 (ViT)
**Time:** ~45 分钟
...
1. **Train on everything.** 680,000 小时 of weakly-labeled audio scraped from the internet across 97 languages. No clean academic corpus. No phoneme labels.
...
3. **Standard encoder-decoder transformer.** 编码器 consumes log-mel spectrograms. 解码器 produces text tokens autoregressively. No vocoder, no CTC, no HMM.
...
| TTS | Piper, XTTS-v2, Kokoro | 编码器-decoder pattern, but Whisper-shaped |
...
1. **Easy.** Run `code/main.py`. Confirm the frame count for a 1-second signal at 16 kHz with 10 ms hop is ~100 frames. 对于 30 seconds: ~3,000 frames.
...
- [HuggingFace blog — "Fine-Tune Whisper 对于 Multilingual ASR with 🤗 Transformers"](https://huggingface.co/blog/fine-tune-whisper) — canonical fine-tuning recipe including mel spectrogram preprocessor and token-timestamp handling.
