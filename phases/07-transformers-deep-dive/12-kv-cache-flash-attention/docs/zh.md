# KV Cache, Flash 注意力机制 & Inference Optimization

> Training is parallel and FLOP-bound. Inference is serial and memory-bound. Different bottleneck, different tricks.

**类型:** 实现
**Languages:** Python
**Prerequisites:** Phase 7 · 02 (Self-注意力机制), Phase 7 · 05 (Full Transformer), Phase 7 · 07 (GPT)
**Time:** ~75 分钟

## The Problem

A naive autoregressive decoder does `O(N²)` work to generate `N` tokens: at each step it recomputes attention over the full prefix. 对于 a 4K-token response that is 16M attention operations, most of them redundant. Every hidden state of a prefix token is deterministic once computed — you only need to run the new token's query against the cached keys and values of everything before.

On top of that, attention itself moves a lot of data. Standard attention materializes an N×N score matrix, N×d softmax output, N×d final output — too many reads and writes to HBM. 对于 N≥2K, attention becomes memory-bound before it becomes FLOP-bound. Classic attention kernels underuse modern GPUs by 4–10×.
...
2. **Flash 注意力机制.** Tile the attention computation so the full N×N matrix never hits HBM. All of softmax + matmul happens in SRAM. 2–4× wall-clock speedup on A100; 5–10× on H100 with FP8.

By 2026 both are universal. Every production inference stack (vLLM, TensorRT-LLM, SGLang, llama.cpp) assumes them. Every frontier model ships with Flash 注意力机制 enabled.

## The Concept

![KV cache growth and Flash 注意力机制 tiling](../assets/kv-cache-flash-attn.svg)
...
对于 a 7B model with 32 layers, 32 heads, d_head=128, fp16:
...
**Numerical trick.** Running softmax maintains `(max, sum)` across tiles so the final normalization is exact. Not an approximation — Flash 注意力机制 computes bit-identical output to standard attention (modulo fp16 non-associativity).

**Version evolution:**

| Version | Year | 键 change | Speedup on reference hardware |
|---------|------|-----------|-------------------------------|
...
Cheap model proposes N tokens. Big model verifies all N in parallel. 如果 verification accepts k tokens, you paid 1 big-model forward pass for k generations. Typical k=3–5 on code and prose.
...
Continuous batching (first shipped in 或者ca, now in vLLM, TensorRT-LLM, SGLang): swap new requests into the batch as soon as old ones finish. 5–10× throughput gain for typical chat workloads.
...
3. A tiled softmax that simulates Flash 注意力机制's running-max algorithm.
...
Prefix caching across requests is a big 2026 win — the same system prompt, few-shot examples, or long context document reuses KV across calls. 对于 agent workloads with repeated tool prompts, prefix caching is routinely 5× throughput gain.
...
3. **Hard.** Implement a toy PagedAttention: KV cache in fixed 16-token blocks with a free-list. 当 a sequence finishes, return its blocks to the pool. Simulate 1,000 chat completions with varying lengths. Compare memory fragmentation vs contiguous allocation.
...
| Flash 注意力机制 | "Tiled attention kernel" | Computes attention without materializing N×N in HBM. |
...
- [Dao et al. (2022). FlashAttention: Fast and Memory-Efficient Exact 注意力机制 with IO-Awareness](https://arxiv.org/abs/2205.14135) — Flash 1.
- [Dao (2023). FlashAttention-2: Faster 注意力机制 with Better Parallelism and Work Partitioning](https://arxiv.org/abs/2307.08691) — Flash 2.
- [Shah et al. (2024). FlashAttention-3: Fast and Accurate 注意力机制 with Asynchrony and Low-precision](https://arxiv.org/abs/2407.08608) — Flash 3.
...
- [Leviathan et al. (2023). Fast Inference from Transformers via 推测解码](https://arxiv.org/abs/2211.17192) — spec decoding.
