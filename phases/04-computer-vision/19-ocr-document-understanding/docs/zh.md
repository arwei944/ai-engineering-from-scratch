...
- Implement CTC (Connectionist Temporal Classification) loss for sequence-to-sequence OCR 训练
...
Images full of text are everywhere: receipts, invoices, IDs, scanned books, forms, whiteboards, signs, screenshots. Extracting structured 数据 from them — not just the characters, but "this is the total amount" — is one of the highest-value applied-vision problems.

The field splits into three skill 层:

1. **OCR proper**: turn 像素 into text.
2. **Layout parsing**: group OCR 输出 into regions (title, body, table, header).
3. **Document understanding**: extract structured fields ("invoice_total = $42.50") from layout.

Each 层 has classical and modern approaches, and the gap between "I want text from an 图像" and "I need the total amount from this receipt" is bigger than most teams realise.
...
OCR recognition produces a variable-length sequence from a fixed-length 特征图. CTC (Graves et al., 2006) lets you train this without character-level alignment. The 模型 输出 a distribution over (vocab + blank) at every time step; CTC loss marginalises over all alignments that reduce to the target text after merging repeats and removing blanks.
...
CTC is the reason CRNN worked in 2015 and still trains most production OCR 模型 in 2026.

### Modern end-to-end 模型

- **Donut** (Kim et al., 2022) — a ViT encoder + a text decoder; reads an 图像 and emits JSON directly. No text detector, no layout module.
- **TrOCR** — ViT + transformer decoder for line-level OCR.
- **Qwen-VL-OCR / InternVL** — full vision-language 模型 fine-tuned for OCR tasks; best accuracy in 2026 on complex documents.
- **PaddleOCR** — classical DB + CRNN pipeline in a mature production package; still the open-source workhorse.

End-to-end 模型 need more 数据 and compute but skip the error accumulation of multi-stage pipelines.
...
For forms, use **Key-Value extraction** 模型 (Donut for visually-rich documents, LayoutLMv3 for plain scans). They take 图像 + detected text + positions and predict structured key-value pairs.
...
Fixed-height 输入 (the CNN max-pools height to 1). Width is the time dimension for CTC.
...
A real OCR 数据集 adds fonts, noise, rotation, blur, and colour. The pipeline above is identical.
...
Loss should drop from ~3 to ~0.2 over 200 steps on this trivial synthetic 数据.
...
- **EasyOCR** — Python-native, multilingual, PyTorch 骨干网络.
- **Tesseract** — classical; still useful for old scanned documents when 模型 struggle.
...
- `输出/prompt-ocr-stack-picker.md` — a prompt that picks Tesseract / PaddleOCR / Donut / VLM-OCR given document type, language, and structure.
- `输出/skill-ctc-decoder.md` — a skill that writes greedy and beam-search CTC decoders from scratch, including length normalisation.

## Exercises

1. **(Easy)** Train the TinyCRNN on 5-digit random numeric strings for 500 steps. Report CER on a held-out set.
2. **(Medium)** Replace greedy decoding with beam search (beam_width=5). Report CER delta. On which 输入 does beam search win?
...
|------|----------------|----------------------|
| OCR | "Text from 像素" | Turning 图像 regions into character sequences |
| CTC | "Alignment-free loss" | Loss that trains a sequence 模型 without per-timestep labels; marginalises over alignments |
| CRNN | "Classic OCR 模型" | Conv 特征 extractor + BiLSTM + CTC; the 2015 baseline still used in production |
| Donut | "End-to-end OCR" | ViT encoder + text decoder; emits JSON directly from 图像 |
...
| VLM-OCR | "LLM that reads" | A vision-language 模型 trained or prompted for OCR tasks; current SOTA on complex documents |
