...
- Explain triplet, contrastive, and proxy-based metric learning losses and pick the right one for a given 数据集
...
- Build a FAISS index, query it by text and by 图像, and report 召回率@K for a held-out query set
- Use DINOv2, CLIP, and SigLIP as off-the-shelf embedding backbones and know when each wins

## The Problem

Retrieval is everywhere in production vision: duplicate detection, reverse 图像 search, visual search ("find similar products"), face re-identification, person re-ID for surveillance, instance-level matching for e-commerce. The product question is always the same: "given this query 图像, rank my catalogue."

Two design decisions shape the whole system. The embedding — what 模型 produces the 向量. The index — how to find nearest neighbours at scale. Both are commodity in 2026 (DINOv2 for the embedding, FAISS for the index), which raises the bar: the hard part is defining *what counts as similar* for your application, then shaping the embedding space so the distances match.

That shaping is metric learning. 它是 a small but high-leverage discipline.
...
| **NT-Xent / InfoNCE** | Pairs + 批次-mined negatives | Scales to large 批次 | Needs big 批次 or momentum queue |
| **Proxy-based (ProxyNCA)** | Class labels only | Fast, stable, no mining | Can overfit to proxies on small 数据集 |

For most production use cases, start with a pretrained 骨干网络 and only add a metric-learning fine-tune if the off-the-shelf embeddings underperform on your test set.
...
Pull anchor `a` close to positive `p`, push it away from negative `n`, with a `margin` that ensures a gap. The three-图像 structure generalises to any similarity ordering.
...
- **Cosine**: angle between 向量. Requires L2-normalised embeddings.
- **L2**: Euclidean distance. Works on raw or normalised embeddings, but is usually paired with L2-normalised + squared L2.

For most modern nets the two are equivalent: `||a - b||^2 = 2 - 2 cos(a, b)` when `||a|| = ||b|| = 1`. Pick the convention that matches your embedding 训练; mixing them silently changes what "nearest" means.
...
Report 召回率@1, @5, @10 side by side. A 召回率@10 above 0.95 with 召回率@1 below 0.5 means the embedding space has the right structure but the ranking is noisy — try longer fine-tunes or a re-ranking step.

For duplicate detection, 精确率@K matters more because every false positive is a user-visible mistake. For visual search, 召回率@K is the product signal.
...
- `IndexFlatIP` / `IndexFlatL2` — brute force, exact, no 训练. Use up to ~1M 向量.
- `IndexIVFFlat` — partition into K cells, search only the closest few cells. Approximate, fast, needs 训练 数据.
- `IndexHNSW` — graph-based, fastest for many queries, large index size.

For 100k 向量 you probably want `IndexFlatIP` on cosine similarity. For 10M you want `IndexIVFFlat`. For 100M+ combined with product quantisation (`IndexIVFPQ`).
...
Always ask which one you are solving before picking a 模型.
...
Given a 批次 of embeddings and labels, find the hardest semi-hard negative for each anchor.
...
- **Milvus / Weaviate / Qdrant** — managed 向量 DB wrappers around FAISS or HNSW.

For SOTA instance retrieval, the recipe is: DINOv2 骨干网络, add an embedding head, fine-tune with a triplet or InfoNCE loss on instance-labelled pairs, index in FAISS.

## Ship It

This lesson produces:

- `输出/prompt-retrieval-loss-picker.md` — a prompt that picks triplet / InfoNCE / ProxyNCA for a given retrieval problem.
- `输出/skill-召回率-at-k-runner.md` — a skill that writes a clean evaluation harness for 召回率@K with train/val/gallery splits and proper 数据 contract.

## Exercises

1. **(Easy)** Run the toy example above. Plot the embeddings with PCA before and after 训练 to see the six clusters form.
2. **(Medium)** Add a ProxyNCA loss implementation: one learned "proxy" per class, standard cross-entropy on cosine similarity. Compare convergence speed vs triplet loss on the toy 数据.
3. **(Hard)** Take 1,000 ImageNet 验证 图像, embed with DINOv2 via HuggingFace, build a FAISS flat index, and report 召回率@{1, 5, 10} against the same 图像 as queries (should be 1.0) and against a held-out split with ImageNet labels as ground truth.

## Key Terms

| Term | What people say | What it actually means |
|------|----------------|----------------------|
| Metric learning | "Shape the space" | 训练 an encoder so distances in its 输出 space reflect a target similarity |
...
| 召回率@K | "Top-K hit rate" | Fraction of queries with at least one correct result in the top K |
| Instance retrieval | "Find this exact thing" | Fine-grained matching; off-the-shelf 特征 usually underperform |
