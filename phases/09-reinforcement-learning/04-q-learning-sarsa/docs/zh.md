...
> 蒙特卡洛 waits until the episode ends. TD updates after every step by bootstrapping the next value estimate. Q-learning is off-policy and optimistic; SARSA is on-policy and cautious. Both are one line of code. Both underpin every deep-RL method in this phase.

**类型:** 实现
**Languages:** Python
**Prerequisites:** Phase 9 · 01 (MDPs), Phase 9 · 02 (动态规划), Phase 9 · 03 (蒙特卡洛)
**Time:** ~75 分钟

## The Problem

蒙特卡洛 works but it has two expensive demands. It needs episodes that terminate, and it only updates after the final return is in. 如果 your episode is 1,000 steps, MC waits 1,000 steps to update anything. It is high-variance, low-bias, and slow in practice.
...
- **状态 hashing.** 如果 states are tuples/tensors, use a hashable key (tuple, not list; tuple of floats rounded, not raw).
...
| GLIE | "Correct exploration schedule" | Greedy in the Limit with Infinite 探索; needed for Q-learning convergence. |
| Bootstrapping | "使用 current estimate in the target" | What distinguishes TD from MC. Source of bias but massive variance reduction. |
...
- [Rummery & Niranjan (1994). On-line Q-learning using connectionist systems](https://www.researchgate.net/publication/2500611_On-Line_Q-Learning_使用_Connectionist_Systems) — the paper that coined SARSA (then called "modified connectionist Q-learning").
