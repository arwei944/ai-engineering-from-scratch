...
Trust Region 策略 Optimization (TRPO, Schulman 2015) was the first fix: constrain each update so the KL divergence between old and new policy stays below `δ`. Theoretically clean, but requires a conjugate-gradient solve per update. Nobody runs TRPO in 2026.
...
- 如果 the advantage `A_t > 0` and the ratio tries to grow past `1 + ε`, the clip flattens the gradient — don't push a good action further than `+ε` above old probability.
- 如果 the advantage `A_t < 0` and the ratio tries to grow past `1 - ε` (meaning we would make a bad action more likely compared to its clipped reduction), the clip caps the gradient — don't push a bad action below `-ε`.
...
4. 对于 `K` epochs, for each minibatch of `(s, a, A, V_target, log π_old(a|s))`:
   - Compute `r_t(θ) = exp(log π_θ(a|s) - log π_old(a|s))`.
   - Apply `L^{CLIP}` + value loss + entropy.
   - Gradient step.
5. Discard the rollout. 回报 to step 1.
...
The "clipped → zero gradient" pattern is the heart of PPO. 如果 the new policy has already drifted too far in the beneficial direction, the update stops.
...
- **Mean KL** `E[log π_old - log π_θ]`. Should stay in `[0, 0.02]`. 如果 it blows past `0.1`, reduce `K_EPOCHS` or `LR`.
- **Clip fraction** — the fraction of samples whose ratio lies outside `[1-ε, 1+ε]`. Should be `~0.1-0.3`. 如果 `~0`, the clip never triggers → raise `LR` or `K_EPOCHS`. 如果 `~0.5+`, you are over-fitting the rollout → lower them.
...
- **对于getting advantage normalization.** Per-batch zero-mean/unit-std normalization is standard. Skipping it wrecks PPO on most benchmarks.
...
- [Schulman et al. (2017). Proximal 策略 Optimization Algorithms](https://arxiv.org/abs/1707.06347) — the paper.
- [Schulman et al. (2015). Trust Region 策略 Optimization](https://arxiv.org/abs/1502.05477) — TRPO, PPO's predecessor.
- [并且rychowicz et al. (2021). What Matters In On-策略 RL? A Large-Scale Empirical Study](https://arxiv.org/abs/2006.05990) — every PPO hyperparameter ablated.
...
- [Engstrom et al. (2020). Implementation Matters in Deep 策略 Gradients](https://arxiv.org/abs/2005.12729) — the "37 code-level optimizations" paper; which PPO tricks are load-bearing and which are folklore.
