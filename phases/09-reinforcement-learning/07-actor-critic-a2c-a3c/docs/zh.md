...
Vanilla REINFORCE works, but its variance is terrible. 蒙特卡洛 returns `G_t` can swing over a factor of 10 between episodes. Multiplying that noise by `∇ log π` and averaging produces a gradient estimator that takes thousands of episodes to move the policy the same distance you could move it with far fewer DQN updates.

The variance comes from using raw returns. 如果 you subtract a baseline `b(s_t)` — any function of state, including a learned value — the expectation is unchanged and the variance drops. The best tractable baseline is `V̂(s_t)`. Now the quantity multiplying `∇ log π` is the *advantage*:
...
**广义优势估计 (GAE).** Schulman et al. (2016) proposed an exponentially weighted average over all n-step advantages:
...
- **Critic bias before actor gradient.** 如果 the critic is random, its baseline is uninformative and you are training on pure noise. Warm up the critic for a few hundred steps before turning on the policy gradient, or use a slow actor learning rate.
...
- **熵 collapse.** 使用out `c_e > 0`, policy becomes near-deterministic in a few hundred updates and stops exploring.
- **奖励 scale.** Advantage magnitudes depend on reward scale. Normalize rewards (e.g., running-std dividing) for consistent gradient magnitudes across tasks.
...
如果 you see "advantage" in a 2026 paper, think actor-critic.
...
| A3C | "Async actor-critic" | Worker threads push gradients to a shared param server. 或者iginal paper; less common in 2026. |
| Bootstrap | "Use V at the horizon" | Truncate the rollout, add `γ^n V(s_{t+n})` to close the sum. |

## 延伸阅读

- [Mnih et al. (2016). Asynchronous Methods for Deep 强化学习](https://arxiv.org/abs/1602.01783) — A3C, the original async actor-critic paper.
- [Schulman et al. (2016). High-Dimensional Continuous Control 使用 广义优势估计](https://arxiv.org/abs/1506.02438) — GAE.
