...
The code here is stdlib-only numpy-free — we use a hand-rolled single-hidden-layer MLP on a tiny continuous GridWorld, so every training step runs in microseconds. 算法是 identical to Atari DQN at scale.
...
对于ward pass: linear → ReLU → linear. That is the entire net.
...
形状是 Q-learning from Lesson 04 with two differences: (a) we backprop through a differentiable `Q(·; θ)` instead of indexing a table, (b) the target uses `Q(·; θ^-)`.

### Step 4: the outer loop

对于 each episode, act ε-greedy on `Q(·; θ)`, push transitions into the buffer, sample a minibatch, take a gradient step, periodically sync `θ^- ← θ`. The pattern:
...
- **探索.** ε must decay, typically from 1.0 to 0.01 over the first ~10% of training. 使用out enough early exploration the Q-net converges to a local basin.
- **Overestimation.** `max` over noisy Q is upward-biased. Always use Double DQN in production.
- **奖励 scale.** Clip or normalize rewards; the gradient magnitude is proportional to reward magnitude.
...
- **观测 preprocessing.** Atari DQN stacks 4 frames to make state Markov. Any env with velocity info needs frame-stacking or recurrent state.
...
The lessons still travel. Replay and target networks appear in SAC, TD3, DDPG, SAC-X, AlphaZero's self-play buffer, and every offline RL method. 奖励 clipping lives on as advantage normalization in PPO. The architecture is the blueprint.
...
- [Mnih et al. (2013). Playing Atari with Deep 强化学习](https://arxiv.org/abs/1312.5602) — the 2013 NeurIPS workshop paper that kicked off deep RL.
...
- [Hasselt, Guez, Silver (2016). Deep 强化学习 with Double Q-learning](https://arxiv.org/abs/1509.06461) — DDQN.
