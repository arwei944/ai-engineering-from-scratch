# 蒙特卡洛 Methods — Learning from Complete Episodes

> Dynamic programming needs a model. 蒙特卡洛 needs nothing but episodes. Run the policy, watch the returns, average them. The simplest idea in RL — and the one that unlocks everything downstream.
...
You need a method that only needs the ability to *sample* from the environment. Run the policy. Get a trajectory `s_0, a_0, r_1, s_1, a_1, r_2, …, s_T`. Use it to estimate values. That is 蒙特卡洛.

The shift from DP to MC is philosophically important: we move from *known model + exact backup* to *sampled rollouts + averaged return*. The variance jumps, but the applicability explodes. Every RL algorithm after this lesson — TD, Q-learning, REINFORCE, PPO, GRPO — is a 蒙特卡洛 estimator at heart, sometimes with bootstrapping layered on top.

## The Concept

![蒙特卡洛: rollout, compute returns, average; first-visit vs every-visit](../assets/monte-carlo.svg)
...
**探索 is now a problem.** DP touched every state by enumeration. MC only sees states the policy visits. 如果 `π` is deterministic, whole regions of the state space never get sampled, and their value estimates stay at zero forever. Three fixes, in historical order:
...
**蒙特卡洛 Control.** Evaluate → improve → evaluate, just like policy iteration, but evaluation is sampling-based:
...
- **Infinite episodes.** MC requires episodes to *terminate*. 如果 your policy can loop forever, cap `max_steps` and treat the cap as implicit failure. GridWorld with a random policy routinely times out — that is normal, just make sure you count it correctly.
...
- **状态 coverage.** Greedy MC on a fresh Q with ties will only ever try one action. You *must* explore (ε-greedy, exploring starts, UCB).
- **Non-stationary policies.** 如果 `π` changes (as in MC control), old returns are from a different policy. Constant-α MC handles this; sample-average MC does not.
...
| 蒙特卡洛 Tree Search (AlphaZero) | MC rollouts from tree leaves guide selection. |
...
|------|-----------------|-----------------------|
| 蒙特卡洛 | "Random sampling" | Estimate expectations by averaging over iid samples from the distribution. |
| 回报 `G_t` | "Future reward" | Sum of discounted rewards from step `t` to episode end: `Σ_{k≥0} γ^k r_{t+k+1}`. |
...
| ε-greedy | "探索 noise" | Pick greedy action with prob `1-ε`; random action with prob `ε`. |
...
- [Sutton & Barto (2018). Ch. 5 — 蒙特卡洛 Methods](http://incompleteideas.net/book/RLbook2020.pdf) — the canonical treatment.
- [Singh & Sutton (1996). 强化学习 with Replacing Eligibility Traces](https://link.springer.com/article/10.1007/BF00114726) — first-visit vs every-visit analysis.
- [Precup, Sutton, Singh (2000). Eligibility Traces for Off-策略 策略 Evaluation](http://incompleteideas.net/papers/PSS-00.pdf) — off-policy MC and variance control.
- [Mahmood et al. (2014). Weighted Importance Sampling for Off-策略 Learning](https://arxiv.org/abs/1404.6362) — modern low-variance IS estimators.
