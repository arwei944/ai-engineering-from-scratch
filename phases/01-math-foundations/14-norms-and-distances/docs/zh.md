...
### 范数s: measuring vector magnitude
...
### Cosine Similarity and Cosine 距离
...
### Wasserstein 距离 (Earth Mover's 距离)
...
### Why Different Tasks Need Different 距离s
...
```
Loss function       距离 it uses       Behavior
...
| Term | 通俗说法 | 实际含义 |
|------|----------------|----------------------|
| 范数 | "Size of a vector" | A function that maps a vector to a non-negative scalar, satisfying triangle inequality, absolute homogeneity, and zero only for the zero vector |
...
| KL divergence | "距离 between distributions" | Not a true distance (not symmetric). Measures extra bits from using Q to encode P |
