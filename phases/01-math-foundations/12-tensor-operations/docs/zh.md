...
> 张量s are the common language between data and deep learning. Every image, every sentence, every gradient flows through them.

**类型:** 实现
**Language:** Python
**前置要求:** 阶段1, Lessons 01 (Linear Algebra Intuition), 02 (Vectors, Matrices & Operations)
**Time:** ~90 minutes
...
PyTorch uses NCHW (channels-first). 张量Flow defaults to NHWC (channels-last). Mismatched layouts cause silent slowdowns or errors.
...
```python
t = 张量(list(range(6)), shape=(1, 3, 1, 2))
s = t.squeeze()
v = 张量([1, 2, 3])
u = v.unsqueeze(0)
```
...
```python
mat = 张量(list(range(6)), shape=(2, 3))
tr = mat.transpose(0, 1)

t4d = 张量(list(range(24)), shape=(1, 2, 3, 4))
...
2. **Medium -- Implement broadcasting.** Extend the `张量` class with a `broadcast_to(shape)` method that expands dimensions of size 1 to match a target shape. Then modify `_elementwise_op` to automatically broadcast before operating. Test with shapes `(3, 1)` and `(1, 4)` producing `(3, 4)`.
...
| Term | 通俗说法 | 实际含义 |
|---|---|---|
| 张量 | "A matrix but more dimensions" | A multi-dimensional array with uniform type and defined shape, strides, and operations |
...
| NCHW / NHWC | "PyTorch vs 张量Flow format" | Memory layout conventions for image tensors. NCHW puts channels before spatial dims, NHWC puts them after |

## Further Reading

- [NumPy Broadcasting](https://numpy.org/doc/stable/user/basics.broadcasting.html) -- The canonical rules with visual examples
- [PyTorch 张量 Views](https://pytorch.org/docs/stable/tensor_view.html) -- When views work and when they copy
