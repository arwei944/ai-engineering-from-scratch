# 注意力机制 Mechanism — The Breakthrough
...
That is the whole idea. Transformers extended it. Self-attention applied it to a single sequence. Multi-head attention ran it in parallel. 但是 the 2014 version already broke the bottleneck, and once you have it, the pivot to transformers is engineering, not conceptual.
...
The weighted average is the point. 当 the decoder needs to translate "Je" to "I", it weights the encoder state over "Je" high and the others low. 当 it needs "not", it weights "pas" high. The context vector reshapes each step.
...
| 注意力机制 score `e_{t,i}` | scalar | One per encoder position |
| 注意力机制 weight `α_{t,i}` | scalar | After softmax over all `i` |
...
Given three encoder states (roughly "cat", "sat", "mat") and a decoder state that aligns most with the first, the attention distribution concentrates on position 0. 如果 the decoder state shifts to align with the last, attention moves to position 2. The context vector tracks.
...
First row wins. Then move the decoder state closer to the third encoder state and watch the weights shift. That is it. 注意力机制 is explicit alignment.
...
### 当 classical attention still matters
...
注意力机制 weights look interpretable. They are weights that sum to one across positions; you can plot them; high means "looked at this." Reviewers love them.
...
|------|-----------------|-----------------------|
| 注意力机制 | Looking at things | Weighted average of a value sequence, weights computed from a query-key similarity. |
...
| Alignment matrix | The pretty picture | 注意力机制 weights as a `(T_dec, T_enc)` grid. Read it to see what the model attended to. |

## 延伸阅读

- [Bahdanau, Cho, Bengio (2014). Neural 机器翻译 by Jointly Learning to Align and Translate](https://arxiv.org/abs/1409.0473) — the paper.
- [Luong, Pham, Manning (2015). Effective Approaches to 注意力机制-based Neural 机器翻译](https://arxiv.org/abs/1508.04025) — the three score variants and their comparison.
- [Jain and Wallace (2019). 注意力机制 is not Explanation](https://arxiv.org/abs/1902.10186) — the interpretability caveat.
- [Dive into Deep Learning — Bahdanau 注意力机制](https://d2l.ai/chapter_attention-mechanisms-and-transformers/bahdanau-attention.html) — runnable walkthrough with PyTorch.
