...
**Type:** Learn
**Languages:** Python
**Prerequisites:** Phase 5 · 13 (问答系统), Phase 5 · 14 (信息检索)
**Time:** ~75 分钟
...
Conversation is hard for an ML system. 输入是 open-ended. The output has to be coherent over many turns. The system may need to act on the world (change a flight, charge a card). Every wrong step is visible to the user.

聊天机器人 architectures have cycled through four paradigms, each introduced because the previous one failed too visibly. This lesson walks them in order. The 2026 production landscape is a hybrid of the last two.

## The Concept

![聊天机器人 evolution: rule-based → retrieval → neural → agent](../assets/chatbot.svg)
...
Threshold-based refusal is the key design choice. 如果 the best match is not close enough, return `None` and let the system escalate.
...
- [Weizenbaum (1966). ELIZA — A Computer Program 对于 the Study of Natural Language Communication](https://web.stanford.edu/class/cs124/p36-weizenabaum.pdf) — the original rule-based chatbot paper.
