...
**类型:** 实现
**Languages:** Python
**Prerequisites:** Phase 5 · 11 (机器翻译), Phase 5 · 10 (注意力机制 Mechanism)
**Time:** ~75 分钟

## The Problem

A user types "当 did the first iPhone launch?" and expects "June 29, 2007." Not "Apple's history is long and varied." Not "2007" sitting in isolation with no sentence. A direct, grounded, correct answer.
...
**Retrieval-augmented (RAG).** Two stages. 首先， a retriever finds the top-`k` passages from a corpus. Second, a reader (extractive or generative) produces the answer using those passages. The retriever-reader split lets each be trained and evaluated independently. Modern RAG often adds a reranker between them.
...
Two-stage pipeline. Dense retriever (Sentence-BERT) finds relevant passages by semantic similarity. Extractive reader (RoBERTa-SQuAD) pulls the answer span from the combined top passages. Works on small corpora. 对于 a million-document corpus, use FAISS or a vector database.
...
- **Refusal calibration.** 当 the answer is not in the retrieved passages, does the system correctly say "I don't know"? Measure false confidence rate.
...
2. **Medium.** Add a refusal classifier. 当 the top retrieval score is below a threshold (say 0.3 cosine), return "I don't know" instead of calling the reader. Tune the threshold on a held-out set.
...
| SQuAD | Canonical benchmark | Stanford 问答系统 Dataset. EM + F1 metrics. |
