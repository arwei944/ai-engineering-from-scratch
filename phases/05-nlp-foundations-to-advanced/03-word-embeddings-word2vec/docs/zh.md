...
**Distributional hypothesis** (Firth, 1957): "You shall know a word by the company it keeps." 如果 two words appear in similar contexts, they probably mean similar things.
...
对于 each positive pair `(center, context)`, sample `k` random words from the vocabulary as negatives. Train the model so the dot product `W[center] · W'[context]` is high for positives and low for negatives.
...
对于 real work, you almost never train Word2Vec yourself. You download pre-trained vectors.
...
1. **Easy.** Run the training loop on a tiny corpus (20 sentences about cats and dogs). After 200 epochs, verify `nearest(vocab, W, W[vocab["cat"]])` returns `dog` in its top 3. 如果 not, increase epochs or vocabulary.
