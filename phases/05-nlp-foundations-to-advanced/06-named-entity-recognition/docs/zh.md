...
对于 classical (non-neural) NER, features are the game. Useful ones:
...
对于 the CRF layer, use `torchcrf.CRF` (pip install pytorch-crf). The gain over hand-crafted CRF is measurable but smaller than you expect unless you have tens of thousands of labeled sentences.
...
`aggregation_strategy="simple"` merges contiguous B-X, I-X tokens into a span. 使用out it, you get token-level labels and have to merge yourself.
...
- **Per-entity-type decomposition.** 对于 long documents, a single call that extracts all entity types at once loses recall as length grows. Run one extraction pass per entity type. Higher inference cost, substantially higher accuracy. This is the standard pattern for clinical notes and legal contracts.
...
- [Lample et al. (2016). Neural Architectures for 命名实体识别](https://arxiv.org/abs/1603.01360) — the BiLSTM-CRF paper. Canonical.
