...
Extractive summarization is a ranking problem. Score every sentence, return the top-`k`. 输出是 always grammatical because it is lifted verbatim. The risk is missing content that is distributed across the article.

Abstractive summarization is a generation problem. A transformer produces new text conditioned on the input. 输出是 fluent and compressive but may hallucinate facts that were not in the source. The risk is confident fabrication.
...
Evaluation with **ROUGE** (Recall-或者iented Understudy for Gisting Evaluation). ROUGE-1 and ROUGE-2 score unigram and bigram overlap. ROUGE-L scores longest common subsequence. Higher is better but 40 ROUGE-L is "good" and 50 is "exceptional." Every paper reports all three. Use the `rouge-score` package.
...
BART-large-CNN is fine-tuned on the CNN/DailyMail corpus. It produces news-style summaries out of the box. 对于 other domains (scientific papers, dialog, legal), use the corresponding Pegasus checkpoint or fine-tune on your target data.
...
Always use stemming. 使用out it, "running" and "run" count as different words and ROUGE undercounts.
...
- **QA-based factuality.** Ask a QA model questions whose answers are in the source. 如果 the summary supports different answers, flag.
- **Entity-level F1.** Compare named entities in source vs summary. Entities present only in the summary are suspect.

对于 anything user-facing where factuality matters (news, medical, legal, financial), extractive is the safer default. Abstractive needs a factuality check in the loop.
...
- [Mihalcea and Tarau (2004). TextRank: Bringing 或者der into Texts](https://aclanthology.org/W04-3252/) — the extractive canonical paper.
...
- [Maynez et al. (2020). On Faithfulness and Factuality in Abstractive 摘要生成](https://arxiv.org/abs/2005.00661) — the factuality landscape paper.
