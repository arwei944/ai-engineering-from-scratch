...
> 如果 a word is surprising, the model is bad. 困惑度 makes surprise a number. Smoothing keeps it finite.
...
4. **Backoff.** 如果 n-gram has count zero, fall back to (n-1)-gram. Katz backoff normalizes this.
...
Sampling proportional to probability. Always gives different output per seed. 对于 beam-search-like output, pick the argmax at each step (greedy) and add a small randomness knob (temperature).
...
Lower is better. 对于 Brown corpus, a well-tuned 4-gram KN model hits perplexity around 140. A transformer LM hits 15-30 on the same test set. The gap is about 10x. That gap is why the field moved on.
...
- **Baselines.** Always compute an n-gram LM perplexity before declaring your neural LM good. 如果 your transformer does not beat KN by a wide margin, something is wrong.
...
| 困惑度 | LM quality metric | `exp(-average log-prob)` on held-out data. Lower is better. |
| Backoff | Fallback to shorter context | 如果 trigram count is zero, use bigram. Katz backoff formalizes this. |
