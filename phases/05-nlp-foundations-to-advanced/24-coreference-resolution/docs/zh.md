...
**Type:** Learn
**Languages:** Python
**Prerequisites:** Phase 5 · 06 (NER), Phase 5 · 07 (POS & 句法分析)
**Time:** ~60 分钟

## The Problem

Extract every mention of Apple Inc. from a 300-word article. Easy when the article says "Apple." Hard when it says "the company," "they," "Cupertino's technology giant," or "Jobs's firm." 使用out resolving these mentions to the same entity, your NER pipeline misses 60-80% of the mentions.
...
- 摘要生成: "The CEO announced..." vs "Tim Cook announced..." — the summary should name the CEO.
...
2. **Mention-pair classifier.** 对于 every pair of mentions (m_i, m_j), predict whether they corefer. Cluster by transitive closure. Standard pre-2016.
3. **Mention-ranking.** 对于 each mention, rank candidate antecedents (including "no antecedent"). Pick the top.
...
- Cataphora (pronoun before referent): "当 **she** walked in, Mary smiled."
...
2. 对于 each pronoun, look at the previous K mentions and score them by:
   - gender/number agreement (heuristic)
   - recency (closer wins)
   - syntactic role (subjects preferred)
3. Link the highest-scoring antecedent.

Not competitive with neural models. 但是 it shows the search space and the decisions an end-to-end model must make.
...
Two failure modes to watch. 首先， LLMs over-merge ("him" and "her" referring to two distinct people). Second, LLMs silently drop mentions in long documents. Always verify with span-offset checks.

### Step 4: evaluation

The standard conll-2012 script computes MUC, B³, CEAF-φ4 and reports the average. 对于 an in-house eval, start with span-level precision and recall on your annotated test set, then add mention-linking F1.
...
| 多语言 | SpanBERT / XLM-R trained on OntoNotes or 多语言 CoNLL |
...
| Cataphora | 对于ward reference | Earlier mention refers to later ("当 he arrived, John..."). |
...
- [Jurafsky & Martin, SLP3 Ch. 26 — 共指消解 and 实体链接](https://web.stanford.edu/~jurafsky/slp3/26.pdf) — canonical textbook chapter.
- [Lee et al. (2017). End-to-end Neural 共指消解](https://arxiv.org/abs/1707.07045) — span-based end-to-end.
