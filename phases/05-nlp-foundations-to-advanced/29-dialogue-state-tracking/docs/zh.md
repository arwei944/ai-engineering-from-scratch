...
- **Classification.** 对于 each (slot, candidate_value) pair, predict yes/no. Works for closed-vocab slots. Standard pre-2020.
...
- **Co-reference across turns.** "让我们 stay with the first option." Needs to resolve which option.
...
Calibrate: what fraction of turns does the system get ALL slots right? 对于 MultiWOZ 2.4, top 2026 systems: 80-83%. Your in-domain system should exceed that on your narrow vocabulary or the LLM baseline beats you.
...
- **Implicit inheritance.** 如果 the user has previously specified "for 4 people," a new request for a different time should not clear people. Always pass the full history.
...
- [Feng et al. (2023). Towards LLM-driven 对话状态跟踪 (LDST)](https://arxiv.org/abs/2310.14970) — LLaMA + LoRA instruction tuning for DST.
...
- [King, Flanigan (2024). Unsupervised End-to-End Task-或者iented Dialogue with LLMs](https://arxiv.org/abs/2404.10753) — EM-based unsupervised TOD.
