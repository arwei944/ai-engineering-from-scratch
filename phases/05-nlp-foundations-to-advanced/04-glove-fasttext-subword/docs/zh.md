# GloVe, FastText, and 子词 Embeddings
...
Each word is represented by its set of n-grams (typically 3 to 6 characters). The word embedding is the sum of its n-gram embeddings. 对于 skip-gram training, plug this in where Word2Vec used a single vector.
...
对于 an unseen word, you still get a vector as long as some of its n-grams are known. `whereupon` shares `<wh`, `her`, `ere`, and `<where` with `where`, so the two land near each other.
...
对于 BPE-style subword tokenization in the transformer era:
...
|------|-----------------|-----------------------|
| 共现 matrix | Word-word frequency table | `X[i][j]` = how often word `j` appears in a window around word `i`. |
| 子词 | Piece of a word | A character n-gram (FastText) or learned token (BPE/WordPiece/SentencePiece). |
...
| Byte-level BPE | BPE on raw bytes | GPT-2's scheme. 词汇表 starts with 256 bytes, so nothing is ever OOV. |
...
- [Bojanowski et al. (2017). Enriching Word Vectors with 子词 Information](https://arxiv.org/abs/1607.04606) — FastText.
- [Sennrich, Haddow, Birch (2016). Neural 机器翻译 of Rare Words with 子词 Units](https://arxiv.org/abs/1508.07909) — the paper that introduced BPE to modern NLP.
