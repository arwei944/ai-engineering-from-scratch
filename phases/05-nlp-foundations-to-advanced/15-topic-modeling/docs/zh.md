...
4. 对于 each cluster, compute class-based TF-IDF over the cluster's documents to extract top words.
...
The filter on `Topic != -1` drops BERTopic's outlier bucket (documents HDBSCAN could not cluster). `min_topic_size` controls HDBSCAN's minimum cluster size; BERTopic's library default is 10. This example sets it to 15 explicitly for the lesson's scale. 对于 corpora over 10,000 documents, increase to 50 or 100.

### Step 3: evaluation

Both methods output topic words. 问题是 whether those words cohere.
...
The biggest practical consideration is document length. BERT embeddings truncate; LDA counts work on whatever length. 对于 documents longer than the embedding model's context, either chunk + aggregate or use LDA.
