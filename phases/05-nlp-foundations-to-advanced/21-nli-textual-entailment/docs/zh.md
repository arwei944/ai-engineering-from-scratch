...
**Type:** Learn
**Languages:** Python
**Prerequisites:** Phase 5 · 05 (情感分析), Phase 5 · 13 (问答系统)
**Time:** ~60 分钟
...
All three problems reduce to 自然语言推理. NLI asks: given a premise `t` and a hypothesis `h`, is `h` entailed by `t`, contradicted, or neutral (unrelated)?
...
对于 production NLI, `facebook/bart-large-mnli` and `microsoft/deberta-v3-large-mnli` are the open defaults. DeBERTa-v3 tops leaderboards.
...
The 2026 meta-pattern: NLI is the duct tape of text understanding. 当ever you need "does A support B?" or "does A contradict B?" — reach for NLI before you reach for another LLM call.
...
|------|-----------------|-----------------------|
| NLI | 自然语言推理 | 3-way classification of premise-hypothesis relationship. |
| RTE | Recognizing 文本蕴含 | Older name for NLI; same task. |
...
- [Williams, Nangia, Bowman (2017). A Broad-Coverage Challenge 语料库 for Sentence 理解 through Inference](https://arxiv.org/abs/1704.05426) — MultiNLI.
...
- [He et al. (2021). DeBERTa: Decoding-enhanced BERT with Disentangled 注意力机制](https://arxiv.org/abs/2006.03654) — the 2026 NLI workhorse.
