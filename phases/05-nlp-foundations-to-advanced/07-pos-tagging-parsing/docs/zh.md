...
Lesson 01 promised that lemmatization needs a part-of-speech tag. 使用out knowing `running` is a verb, a lemmatizer cannot reduce it to `run`. 使用out knowing `better` is an adjective, it cannot reduce to `good`.
...
The dumbest POS tagger that works. 对于 each word, predict the tag it had most often in training.
...
- **词形还原.** Lesson 01 needs POS to lemmatize correctly. Always.
...
- **Query understanding.** "movies directed by Wes 并且erson starring Bill Murray" decomposes into structured constraints via the parse.
...
- **Low-compute pipelines.** 如果 you cannot ship a transformer, POS + dependency parse + gazetteer gets you surprisingly far.
...
1. **Easy.** 使用 the most-frequent-tag baseline on a small tagged corpus (e.g., NLTK's Brown subset), measure accuracy on held-out sentences. Verify the ~85% result.
...
| Universal Dependencies | 多语言 tagset | Coarser than PTB; language-neutral; defaults for cross-lingual work. |
