...
2. **Language coverage.** Monolingual English models still win on English-only tasks. 多语言 models win when corpora are mixed.
...
**Matryoshka Representation Learning.** Trained so the first N dimensions of the vector form a useful standalone embedding. Truncate a 1,536-dim vector to 256 dim and pay ~1% accuracy for 6× storage savings. Supported by OpenAI text-3, Cohere v4, Voyage-4, Jina v5, Gemini 嵌入 2, Nomic v1.5+.

### The MTEB leaderboard tells a partial story

Massive Text 嵌入 Benchmark — 56 tasks across 8 task types at launch (2022), expanded to 100+ tasks in MTEB v2. In early 2026, Gemini 嵌入 2 tops retrieval (67.71 MTEB-R). Cohere embed-v4 leads general (65.2 MTEB). BGE-M3 leads open-weight multilingual (63.0). The leaderboard is necessary but not sufficient — always benchmark on your domain.
...
| Open-weight, multilingual | `BAAI/bge-m3` or `Qwen3-嵌入-8B` |
| Long context (32k+) | Voyage-3-large, Cohere embed-v4, Qwen3-嵌入-8B |
...
| MTEB | The benchmark | Massive Text 嵌入 Benchmark — 56 tasks at launch, 100+ in v2. |
...
- [Muennighoff et al. (2022). MTEB: Massive Text 嵌入 Benchmark](https://arxiv.org/abs/2210.07316) — the leaderboard paper.
