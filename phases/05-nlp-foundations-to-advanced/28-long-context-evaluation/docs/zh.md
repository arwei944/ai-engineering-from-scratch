...
**Type:** Learn
**Languages:** Python
**Prerequisites:** Phase 5 · 13 (问答系统), Phase 5 · 23 (分块 Strategies)
**Time:** ~60 分钟

## The Problem

You have a 200-page contract. The model claims a 1M-token context. You paste the contract in and ask: "什么是 the termination clause?" The model answers — but answers from the cover page because the termination clause sits at 120k tokens deep, past where the model actually attends.
...
**MRCR (Multi-Round 共指消解).** Multi-turn coreference at scale. 8-needle, 24-needle, 100-needle variants. Exposes how many facts a model can juggle before attention degrades.
...
- **Lexical overlap with filler.** 如果 the needle shares keywords with the filler, retrieval becomes trivial. Use NoLiMa-style non-overlapping needles.
