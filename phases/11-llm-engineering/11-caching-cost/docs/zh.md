...
**Time:** ~45 minutes
**Related:** Phase 11 · 15 (Prompt 缓存) — this lesson covers application-layer 缓存 (semantic cache, exact hash cache, model routing). Lesson 15 covers provider-layer prompt 缓存 (Anthropic cache_control, OpenAI automatic, Gemini CachedContent). Combine both for 50-95% cost reduction.

## 学习目标

- Implement semantic 缓存 that serves repeated or similar queries from cache instead of making a new API call
- Calculate per-request costs across providers and implement token-aware rate limiting and budget alerts
- Build a 成本优化 layer with prompt compression, model routing (expensive vs cheap), and response 缓存
- Design a tiered 缓存 strategy using exact match, semantic similarity, and prefix 缓存 for different query types
...
That is just the LLM. Add 嵌入s, vector database hosting, infrastructure. You are looking at $30,000/month for a chatbot.
...
All three major providers offer provider-side prompt 缓存 in 2026, but the mechanics differ. See Phase 11 · 15 for the deep dive.
...
**OpenAI's approach** is automatic. Any prompt prefix that matches a previous request gets a 50% discount. No markers needed. The 权衡: less discount, less control, but zero implementation effort.

### Semantic 缓存: Your Custom Layer

Provider 缓存 only works for identical prefixes. Semantic 缓存 handles the harder case: different queries with the same meaning.
...
The 嵌入 costs are negligible. OpenAI's text-嵌入-3-small costs $0.02 per million tokens. Checking the cache costs almost nothing compared to a full LLM call.

### Exact 缓存: Hash and Match

For deterministic calls (temperature=0, same model, same prompt), exact 缓存 is simpler and faster. Hash the full prompt, check the cache, return if found.

This works perfectly for:
- System prompt + fixed context + identical user queries
- 函数调用 with identical tool definitions
...
This data reveals which features are expensive, which users are heavy consumers, and where 缓存 has the most impact.
...
|-------|-----------|----------------|----------------------|
| 1 | Provider prompt 缓存 | 30-50% | Low (add cache markers) |
| 2 | Exact 缓存 | 10-20% | Low (hash + dict) |
| 3 | Semantic 缓存 | 15-30% | Medium (嵌入s + similarity) |
...
| Monthly 嵌入 cost | $0 | $180 | (new cost) |
| Total monthly cost | $22,500 | $5,380 | 76% |

The 嵌入 cost for semantic 缓存 ($180/month) pays for itself within the first hour of cache hits.
...
Batch API gives a flat 50% discount on all tokens. Results arrive within 24 hours. Perfect for non-real-time workloads: 评估s, data labeling, bulk summarization.

### 生产环境 Semantic Cache with Redis
...
In 生产环境, replace the linear scan with a vector index (Redis Vector Search, Pinecone, or pgvector). Linear scan works for <1,000 entries. Beyond that, use ANN (approximate nearest neighbor) for O(log n) lookup.

## Ship It

This lesson produces `outputs/prompt-cost-optimizer.md` -- a reusable prompt that analyzes your LLM application and recommends specific 成本优化s with projected savings.

It also produces `outputs/skill-cost-patterns.md` -- a decision framework for choosing the right 缓存 strategy, rate limiting configuration, and model routing rules for your use case.
...
3. **Implement tiered semantic 缓存.** Use two similarity thresholds: 0.98 for high-confidence hits (return immediately) and 0.90 for medium-confidence hits (return with a disclaimer: "Based on a similar previous question..."). Track which tier each hit came from and measure user satisfaction differences.

4. **Build a model routing classifier.** Replace the keyword-based classifier with an 嵌入-based one. Embed 50 labeled queries (simple/medium/complex), then classify new queries by finding the nearest labeled example. Measure classification accuracy against a test set of 20 queries.
...
|------|----------------|----------------------|
| Prompt 缓存 | "Cache the system prompt" | Provider-level 缓存 where repeated prompt prefixes get a discount (90% Anthropic, 50% OpenAI) -- no code changes for OpenAI, explicit markers for Anthropic |
| Semantic 缓存 | "Smart 缓存" | 嵌入 the query, computing similarity to past queries, and returning the cached response if similarity exceeds a threshold -- catches paraphrases that exact matching misses |
| Exact 缓存 | "Hash 缓存" | Hashing the full prompt (model + messages + temperature) and returning the cached response for identical inputs -- only works for temperature=0 deterministic calls |
...
| Cache hit rate | "Cache efficiency" | The percentage of requests served from cache instead of calling the LLM -- 40-60% is typical for 生产环境 chatbots, saves proportionally on cost |

## Further Reading

- [Anthropic Prompt 缓存 Guide](https://docs.anthropic.com/en/docs/build-with-claude/prompt-缓存) -- the official docs for Anthropic's explicit cache_control markers, pricing, and cache lifetime behavior
- [OpenAI Prompt 缓存](https://platform.openai.com/docs/guides/prompt-缓存) -- OpenAI's automatic 缓存, how to verify cache hits via usage fields, and minimum prefix lengths
...
- [GPTCache](https://github.com/zilliztech/GPTCache) -- open-source semantic 缓存 library supporting multiple 嵌入 backends, vector stores, and eviction policies
- [Martian Model Router](https://docs.withmartian.com) -- 生产环境 model routing that automatically selects the cheapest model capable of handling each query
- [Not Diamond](https://www.notdiamond.ai) -- ML-based model router that learns from your traffic patterns to optimize cost/quality 权衡 across providers
- [Helicone](https://www.helicone.ai) -- LLM observability platform with cost tracking, 缓存, rate limiting, and budget alerts as a proxy layer
...
- [Kwon et al., "Efficient Memory Management for Large Language Model Serving with PagedAttention" (SOSP 2023)](https://arxiv.org/abs/2309.06180) -- the vLLM paper; why paged KV-cache + continuous batching beat naive servers 24× on throughput, the infra layer under "缓存 and cost."
- [Dao et al., "FlashAttention-2: Faster Attention with Better Parallelism and Work Partitioning" (ICLR 2024)](https://arxiv.org/abs/2307.08691) -- kernel-level cost reduction orthogonal to prompt 缓存; read alongside speculative decoding and GQA for the full cost-curve picture.
