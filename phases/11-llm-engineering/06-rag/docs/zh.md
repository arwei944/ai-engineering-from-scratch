...
> Your LLM knows everything up to its training cutoff. It knows nothing about your company's docs, your codebase, or last week's meeting notes. RAG solves this by retrieving relevant documents and stuffing them into the prompt. It's the most deployed pattern in 生产环境 AI. If you build one thing from this course, build a RAG pipeline.

**类型:** 实现
**Languages:** Python
**Prerequisites:** Phase 10 (LLMs from Scratch), Phase 11 Lessons 01-05
**Time:** ~90 minutes
**Related:** Phase 5 · 23 (Chunking Strategies for RAG) for the six chunking algorithms and when each wins. Phase 5 · 22 (嵌入 Models Deep Dive) for picking the embedder. Phase 11 · 07 (Advanced RAG) for hybrid search, reranking, and query transformation.

## 学习目标

- Build a complete RAG pipeline: document loading, chunking, 嵌入, vector storage, retrieval, and generation
- Implement semantic search using a vector database (ChromaDB, FAISS, or Pinecone) with proper indexing
- Explain why RAG is preferred over 微调 for knowledge-grounded applications (cost, freshness, attribution)
...
Query -> Retrieve -> Augment prompt -> Generate. Every RAG system follows this pattern. The differences between 生产环境 RAG systems are in the details of each step: how you chunk, how you embed, how you search, and how you construct the prompt.

### Why RAG Beats 微调

| Concern | 微调 | RAG |
|---------|------------|-----|
| Cost | $1,000-$100,000+ per training run | $0.01-$0.10 per query (嵌入 + LLM) |
...
微调 changes the model's weights permanently. RAG changes the model's context temporarily. For most applications, temporary context is what you want.

The one case where 微调 wins: when you need the model to adopt a specific style, tone, or reasoning pattern that cannot be achieved through prompting alone. For factual knowledge retrieval, RAG wins every time.

### 嵌入 Models

An 嵌入 model converts text into a dense vector. Similar texts produce vectors that are close together in this high-dimensional space. "How do I reset my password?" and "I need to change my password" produce nearly identical vectors despite sharing few words. "The cat sat on the mat" produces a very different vector.

Common 嵌入 models (2026 lineup — see Phase 5 · 22 for full analysis):
...
| Gemini 嵌入 2 | 3072 (Matryoshka) | Google | Top MTEB retrieval; 8K context |
...
| Qwen3-嵌入 | 4096 (Matryoshka) | Alibaba (open-weight) | Top open-weight retrieval score |
| all-MiniLM-L6-v2 | 384 | Open-weight (Sentence Transformers) | Prototyping baseline |

For this lesson, we build our own simple 嵌入 using TF-IDF. Not because TF-IDF is what 生产环境 systems use, but because it makes the concept concrete: text goes in, a vector comes out, similar texts produce similar vectors.
...
Documents are too long to embed as single vectors. A 50-page PDF might produce a terrible 嵌入 because it contains dozens of topics. Instead, you split documents into chunks and embed each chunk separately.
...
Most 生产环境 RAG systems use 256-512 token chunks with 50-token overlap. Anthropic's RAG guidelines recommend this range.
...
The indexing phase runs once per document (or when documents update). The querying phase runs on every user request. In 生产环境, indexing might process millions of documents over hours. Querying must respond in under a second.

### Real Numbers

Most 生产环境 RAG systems use these parameters:
...
- **嵌入 dimension**: 384-3072 depending on model
- **Indexing throughput**: 100-1,000 documents per second with API 嵌入s
...
In 生产环境, this is where you call the LLM API. For this lesson, we simulate generation by extracting the most relevant sentence from the retrieved context.
...
With a real 嵌入 model and LLM, the code barely changes:
...
The pipeline is the same. Swap the 嵌入 function. Swap the generation function. The retrieval logic, chunking, prompt construction -- all identical regardless of which models you use.
...
1. Replace the TF-IDF 嵌入s with a simple bag-of-words approach (binary: 1 if word present, 0 if not). Compare retrieval quality on the sample documents. TF-IDF should outperform because it weights rare words higher.
...
4. Implement a simple 评估: given 10 question-answer pairs, run each question through the RAG pipeline, and measure what percentage of retrieved chunks contain the answer. This is retrieval recall at k.
...
| 嵌入 | "Convert text to numbers" | A dense vector representation of text where similar meanings produce similar vectors |
...
| Indexing | "Preparing docs for search" | The offline process of chunking, 嵌入, and storing documents so they can be searched at query time |
...
- Anthropic's RAG documentation (docs.anthropic.com) -- practical guidelines for chunk sizes, prompt construction, and 评估
- Pinecone Learning Center, "What is RAG?" -- clear visual explanations of the RAG pipeline with 生产环境 considerations
- Sentence-BERT: Reimers & Gurevych (2019) -- the paper behind the all-MiniLM 嵌入 models, showing how to train bi-encoders for semantic similarity
