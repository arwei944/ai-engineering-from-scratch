...
> Full 微调 a 7B model requires 56GB of VRAM. You don't have that. Neither do most companies. LoRA lets you fine-tune the same model in 6GB by training less than 1% of the parameters. This isn't a compromise -- it matches full 微调 quality on most tasks. The entire open-source 微调 ecosystem runs on this one trick.
...
- Calculate the parameter savings of LoRA vs full 微调: rank r with d_model dimensions trains 2*r*d parameters instead of d^2
...
There's a deeper problem too. Full 微调 modifies every weight in the model. If you fine-tune on customer support data, you might degrade the model's general capabilities. It's called catastrophic forgetting. The model gets better at your task and worse at everything else.
...
LoRA at r=16 is within 1% of full 微调 on most benchmarks. QLoRA at r=16 loses another fraction of a percent. QLoRA at r=64 essentially matches full 微调 while using 90% less memory.

### Real-World Costs

微调 Llama 3 8B on 50,000 examples (3 epochs):
...
QLoRA on a single consumer GPU costs less than a lunch. This is why the open-weight 微调 community exploded in 2023 and why every training framework below ships QLoRA by default in 2026.
...
| **LLaMA-Factory** | GUI/CLI/API over PEFT + TRL | You want zero-code 微调; 100+ model families supported |
| **torchtune** | Native PyTorch recipes, no `transformers` dep | You want minimal deps and your org already standardizes on PyTorch |

Rule of thumb: research use or one-off experiment → PEFT. Repeatable 生产环境 pipeline → Axolotl with Unsloth kernels enabled. Throwaway prototyping → LLaMA-Factory.
...
微调 is the third option, not the first.

**First: 提示词工程.** Write a better system prompt. Add 少样本 examples. Use chain-of-thought. This costs nothing and takes minutes. If prompting gets you 80% of the way there, you probably don't need to fine-tune.
...
**Third: 微调.** Use this when you need the model to adopt a specific style, format, or reasoning pattern that cannot be achieved through prompting. When you need consistent 结构化输出. When you need to distill a larger model into a smaller one. When latency matters and you can't afford the extra tokens from 少样本 prompting.
...
This simulates 4-bit quantization by mapping weights into 16 discrete levels within blocks of 64. 生产环境 QLoRA uses the bitsandbytes library for true NF4 on GPU.
...
- `outputs/skill-微调-guide.md` -- a skill that teaches agents the decision tree for when and how to fine-tune
...
|------|----------------|----------------------|
| LoRA | "Efficient 微调" | Low-Rank Adaptation: freeze base weights, train two small matrices A and B whose product approximates the full weight update |
| QLoRA | "Fine-tune on a laptop" | Quantized LoRA: load the base model in 4-bit NF4, train LoRA adapters in fp16 on top, enabling 7B 微调 in 6GB VRAM |
...
| Catastrophic forgetting | "微调 broke everything else" | When updating all weights causes the model to lose previously learned capabilities |
...
- Dettmers et al., "QLoRA: Efficient Finetuning of Quantized Language Models" (2023) -- introduces NF4, double quantization, and paged optimizers, enabling 65B 微调 on a single 48GB GPU
...
- [Unsloth documentation](https://docs.unsloth.ai/) -- fused kernels that double 微调 throughput and halve memory; the performance layer under TRL.
