# Building a 生产环境 LLM Application

> You have built prompts, 嵌入s, RAG pipelines, 函数调用, 缓存 layers, and 护栏. Separately. In isolation. Like practicing guitar scales without ever playing a song. This lesson is the song. You will wire every component from Lessons 01-12 into a single 生产环境-ready service. Not a toy. Not a demo. A system that handles real traffic, fails gracefully, streams tokens, tracks costs, and survives its first 10,000 users.

**类型:** 实现 (Capstone)
**Languages:** Python
**Prerequisites:** Phase 11 Lessons 01-15
**Time:** ~120 minutes
**Related:** Phase 11 · 14 (MCP) for replacing bespoke tool schemas with a shared protocol; Phase 11 · 15 (Prompt 缓存) for 50-90% cost reduction on stable prefixes. Both are expected in every serious 2026 生产环境 stack.

## 学习目标

- Wire all Phase 11 components (prompts, RAG, 函数调用, 缓存, 护栏) into a single 生产环境-ready service
...
Every LLM application in 生产环境 today -- Perplexity, Cursor, ChatGPT, Notion AI -- solved these problems. Not by being smarter about prompts. By being rigorous about engineering.

This is the capstone. You will build a complete 生产环境 LLM service that integrates prompt management (L01-02), 嵌入s and vector search (L04-07), 函数调用 (L09), 评估 (L10), 缓存 (L11), 护栏 (L12), streaming, error handling, observability, and cost tracking. One service. Every component wired together.
...
The request enters through an API gateway that handles authentication and rate limiting. Input 护栏 check for prompt injection and banned content before the prompt router selects the right template. A semantic cache checks if a similar question was answered recently. On a cache miss, the LLM is called with streaming enabled. Output 护栏 validate the response. The eval logger records quality metrics. The cost tracker accounts for every token. The response streams back to the client.
...
| 嵌入s | L04 | text-嵌入-3-small | Semantic similarity for cache and RAG |
| Vector Store | L06-07 | In-memory (prod: Pinecone/Qdrant) | Nearest neighbor search for context retrieval |
| 函数调用 | L09 | Tool registry + JSON Schema | External data access, structured actions |
| 评估 | L10 | Custom metrics + logging | Response quality, latency, accuracy tracking |
| 缓存 | L11 | Semantic cache (嵌入-based) | Avoid redundant LLM calls, reduce cost and latency |
| 护栏 | L12 | Regex + classifier rules | Block prompt injection, PII, unsafe content |
...
生产环境 LLM apps fail in three distinct ways. Each requires a different recovery strategy.
...
You cannot improve what you cannot see. Every 生产环境 LLM app needs three pillars of observability.
...
**Tracing.** A single user request touches 5-8 components. OpenTelemetry traces let you see the full journey: how long did 嵌入 take? Was it a cache hit? How long was the LLM call? Did the guardrail add latency? Without tracing, debugging 生产环境 issues is guesswork.
...
**Cursor.** The open file, surrounding files, recent edits, and terminal output form the context. A prompt router decides: small model for autocomplete (Cursor-small, ~20ms), large model for chat (Claude Sonnet 4.6 / GPT-5, ~3s). Context is aggressively compressed -- only relevant code sections, not entire files. Codebase 嵌入s provide long-range context. Speculative edits stream diffs, not full files. MCP integration lets third-party tools plug in without per-tool code changes.

**ChatGPT.** Plugins, 函数调用, and MCP servers let the model access the web, run code, generate images, and query databases. A routing layer decides which capabilities to invoke. Memory persists user preferences across sessions. The system prompt is 1,500+ tokens of behavioral rules, cached via prompt 缓存. Multiple models serve different features: GPT-5 for chat, GPT-Image for images, Whisper for voice, o4-mini for deep reasoning.
...
Without 缓存, the same traffic costs $11,625/month. A 35% cache hit rate saves 35% on LLM costs. This is why Lesson 11 exists.
...
| 3 | Input 护栏 active (prompt injection, PII) | Safety |
| 4 | Output 护栏 active (content filtering, format validation) | Safety |
...
| 14 | CORS configured for 生产环境 domains only | Security |
...
The code builds a complete 生产环境 LLM service with:
- FastAPI server with health checks and CORS
- Prompt template management with versioning and A/B testing
- Semantic 缓存 using cosine similarity on 嵌入s
- Input and output 护栏 (prompt injection, PII, content safety)
...
- 评估 logging for quality tracking
...
嵌入-based cache that matches semantically similar queries. Two questions phrased differently but meaning the same thing will hit the cache.
...
### FastAPI Server (生产环境 Deployment)

The demo above runs as a script. For 生产环境, wrap it in FastAPI with proper endpoints.
...
This lesson produces `outputs/prompt-architecture-reviewer.md` -- a reusable prompt that reviews the architecture of any LLM application against the 生产环境 checklist. Give it a description of your system and it returns a gap analysis.

It also produces `outputs/skill-生产环境-checklist.md` -- a decision framework for shipping LLM applications to 生产环境, covering every component from this lesson with specific thresholds and pass/fail criteria.
...
2. **Implement real 函数调用.** Add a tool registry (from Lesson 09) to the service. When a user asks a question that requires external data (weather, calculation, search), the pipeline should detect this, execute the tool, and include the result in the prompt. Add a `tools_used` field to the response.
...
| Semantic Cache | "Smart cache" | A cache keyed by 嵌入 similarity rather than exact string match -- two differently-phrased identical questions return the same cached response |
...
| Graceful Degradation | "Partial failure handling" | When a secondary component fails (cache, RAG, 护栏), the system continues with reduced functionality rather than crashing |
...
| Health Check | "Readiness probe" | An endpoint that returns the status of all dependencies (cache, LLM availability, 护栏) -- used by load balancers and Kubernetes to route traffic |
...
- [OpenAI 生产环境 Best Practices](https://platform.openai.com/docs/guides/生产环境-best-practices) -- rate limits, error handling, and scaling guidance from the largest LLM API provider
...
- [Semantic 缓存 with GPTCache](https://github.com/zilliztech/GPTCache) -- 生产环境 semantic 缓存 library that implements the concepts from this lesson at scale
- [Hamel Husain, "Your AI Product Needs Evals"](https://hamel.dev/blog/posts/evals/) -- the definitive guide on 评估-driven development for LLM applications, complementing the eval component in this capstone
- [Eugene Yan, "Patterns for Building LLM-based Systems"](https://eugeneyan.com/writing/llm-patterns/) -- architectural patterns (护栏, RAG, 缓存, routing) seen across 生产环境 LLM deployments at major tech companies
