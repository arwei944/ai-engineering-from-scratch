...
The next step is obvious once you see it. The agent is already a 状态机 — system prompt plus message history plus pending tool calls plus the next action. Make the 状态机 explicit: nodes for "the model thinks," "a tool runs," "a human approves," and edges for the conditional transitions between them. Once the graph is explicit, the harness gets four things for free: checkpointing (save state between steps), interrupts (pause for a human), streaming (stream tokens and intermediate events), and time-travel (rewind to a prior state and try a different branch).

LangGraph is the library that ships this abstraction. It is not an 智能体框架 in the LangChain sense ("here is an AgentExecutor, good luck"). It is a graph runtime with first-class state, first-class persistence, and first-class interrupts. The agent loop is something you draw, not something you hand-write.
...
You compile the graph. Compile binds the topology, attaches a checkpointer (optional but essential for 生产环境), and returns a runnable. You invoke it with an initial state and a `thread_id`. Every step of execution persists a checkpoint keyed on `(thread_id, checkpoint_id)`.
...
**Time-travel.** `graph.get_state_history(thread_id)` returns the full checkpoint log. Pass any prior `checkpoint_id` to `graph.invoke` and you fork from that point. Great for debugging ("what if the model had picked tool B instead?") and for regression tests that replay 生产环境 traces.
...
A 生产环境 ReAct agent is four nodes and two edges:
...
### Step 5: swap the checkpointer for 生产环境
...
- Phase 11 · 09 (函数调用) — the tool-call primitive every LangGraph agent node reuses.
- Phase 11 · 14 (模型上下文协议) — external tool discovery that plugs into a LangGraph `ToolNode` via the MCP adapter.
- Phase 11 · 17 (智能体框架 权衡) — when to pick LangGraph over CrewAI, AutoGen, or Agno.
