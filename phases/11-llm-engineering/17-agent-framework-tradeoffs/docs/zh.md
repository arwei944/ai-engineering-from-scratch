# 智能体框架 权衡 — LangGraph vs CrewAI vs AutoGen vs Agno
...
|-----------|------------------|----------|-----------|
| **LangGraph** | `StateGraph` — typed state, nodes, conditional edges, checkpointer. | Workflows with explicit state and human-in-the-loop interrupts; 生产环境 agents needing time-travel debugging. | Loose, role-driven brainstorming where the topology is unknown. |
...
- **LangGraph** → you draw a graph. Nodes are steps, edges are transitions, and the state object at every point is typed. The mental model is a 状态机.
...
State is where most framework choices break down in 生产环境.
...
- Phase 11 · 22 (生产环境 observability) — how to instrument whichever framework you pick.
