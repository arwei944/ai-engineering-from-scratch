...
> LLMs cannot do anything. They generate text. That is the entire capability. They cannot check the weather, query a database, send an email, run code, or read a file. Every "AI agent" you have ever seen is an LLM generating JSON that says which function to call -- and then your code actually calling it. The model is the brain. Tools are the hands. 函数调用 is the nervous system connecting them.

**类型:** 实现
**Languages:** Python
**Prerequisites:** Phase 11 Lesson 03 (结构化输出s)
**Time:** ~75 minutes
**Related:** Phase 11 · 14 (模型上下文协议) — when a tool is shared across hosts, graduate from inline function-calling to an MCP server. This lesson covers the inline case; MCP covers the protocol case.

## 学习目标

- Implement a 函数调用 loop: define tool schemas, parse the model's tool-call JSON, execute functions, and return results
...
- Handle 函数调用 edge cases: parallel tool calls, error propagation, and preventing infinite tool loops
...
This is 函数调用. The model outputs structured JSON describing which function to invoke with what arguments. Your application executes the function. The result goes back into the conversation. The model uses the result to produce its final answer.
...
Every major provider supports 函数调用, but the API surface differs.
...
Lesson 03 covered 结构化输出s. 函数调用 uses the same JSON Schema machinery, but for a different purpose.

**结构化输出s**: force the model to produce data in a specific shape. The output is the final product. Example: extract product info from text as `{name, price, in_stock}`.

**函数调用**: the model declares an intent to execute an action. The output is an intermediate step. Example: `get_weather(city="Tokyo")` -- the model is requesting an action, not producing the final answer.

Use 结构化输出s when you want data extraction. Use 函数调用 when you want the model to interact with external systems.

### Security: The Non-Negotiable Rules

函数调用 is the most dangerous capability you can give an LLM. The model chooses what to execute. If your tool set includes database queries, the model constructs the queries. If it includes shell commands, the model writes them.
...
MCP is to 函数调用 what HTTP is to networking. It standardizes the transport layer so tools become portable.
...
It also produces `outputs/skill-function-calling-patterns.md` -- a decision framework for implementing 函数调用 in 生产环境, covering tool design, error handling, security, and provider-specific patterns.
...
5. **Implement tool call 缓存.** If the same tool is called with identical arguments within 60 seconds, return the cached result instead of re-executing. Use a dictionary keyed by `(tool_name, frozenset(args.items()))`. Measure cache hit rates across a conversation with 20 queries.

## Key Terms

| Term | What people say | What it actually means |
|------|----------------|----------------------|
| 函数调用 | "Tool use" | The model outputs structured JSON describing a function to invoke with specific arguments -- your code executes it, not the model |
...
| MCP | "Tool protocol" | 模型上下文协议 -- Anthropic's open standard for exposing tools via servers that any compatible client can discover and call |
...
- [OpenAI 函数调用 Guide](https://platform.openai.com/docs/guides/function-calling) -- the definitive reference for tool use with GPT-4o, including parallel calls, forced calling, and structured arguments
...
- [模型上下文协议 Specification](https://modelcontextprotocol.io) -- the open standard for tool interoperability across AI applications, with server/client architecture
...
- [Patil et al., 2023 -- "Gorilla: Large Language Model Connected with Massive APIs"](https://arxiv.org/abs/2305.15334) -- 微调 LLMs for accurate API calls across 1,645 APIs with hallucination reduction
- [Berkeley 函数调用 Leaderboard](https://gorilla.cs.berkeley.edu/leaderboard.html) -- real-time benchmark comparing 函数调用 accuracy across GPT-4o, Claude, Gemini, and open models
