# Prompt 缓存 and Context 缓存

> Your system prompt is 4,000 tokens. Your RAG context is 20,000 tokens. You send both with every request. You also pay for both — every time. Prompt 缓存 lets the provider keep that prefix warm on their side and bill you 10% of the normal rate on reuse. Used correctly, it cuts inference cost by 50–90% and first-token latency by 40–85%.

**类型:** 实现
**Languages:** Python
**Prerequisites:** Phase 11 · 01 (提示词工程), Phase 11 · 05 (上下文工程), Phase 11 · 11 (缓存 and Cost)
**Time:** ~60 minutes
...
That move is prompt 缓存. Anthropic shipped it in August 2024 (with a 1-hour extended-TTL variant in 2025), OpenAI automated it later that year, Google shipped explicit context 缓存 alongside Gemini 1.5, and all three now offer it as a first-class feature on their frontier models.
...
Violate the order — put the user message above the system prompt, interleave dynamic retrievals between 少样本s — and the cache never hits.
...
### Step 1: Anthropic prompt 缓存 with explicit markers
...
### Step 4: Gemini explicit context 缓存
...
### Step 5: measuring hit rate in 生产环境

See `code/main.py` for a simulated three-provider accountant that tracks write/read/miss counts and computes blended cost per 1K requests. Gate deploys on a target hit rate — most 生产环境 Anthropic setups should see >80% read fraction after warmup.
...
Combine with semantic 缓存 (Phase 11 · 11) for the user-message layer: prompt 缓存 handles *token-identical* reuse, semantic 缓存 handles *meaning-identical* reuse.

## Ship It

Save `outputs/skill-prompt-缓存-planner.md`:
...
|------|-----------------|-----------------------|
| Prompt 缓存 | "Makes long prompts cheap" | Reusing a provider-side KV-cache for matching prefixes; 50-90% discount on repeated input tokens. |
...
| Context 缓存 (Gemini) | "The explicit one" | Google's named, storage-billed cache object; best for multi-day reuse of large corpora. |

## Further Reading

- [Anthropic — Prompt 缓存](https://docs.anthropic.com/en/docs/build-with-claude/prompt-缓存) — `cache_control`, 1h TTL, break-even tables.
- [OpenAI — Prompt 缓存](https://platform.openai.com/docs/guides/prompt-缓存) — automatic prefix matching.
- [Google — Context 缓存](https://ai.google.dev/gemini-api/docs/缓存) — `CachedContent` API and storage pricing.
- [Anthropic engineering — Prompt 缓存 for long-context workloads](https://www.anthropic.com/news/prompt-缓存) — original launch post with latency numbers.
- Phase 11 · 05 (上下文工程) — where to slice the prompt so the cache can land.
- Phase 11 · 11 (缓存 and Cost) — pair prompt 缓存 with a semantic cache on user messages.
- [Pope et al., "Efficiently Scaling Transformer Inference" (2022)](https://arxiv.org/abs/2211.05102) — the KV-cache memory model that prompt 缓存 exposes to users; explains why a cached prefix is ~10× cheaper to reread than to recompute.
- [Agrawal et al., "SARATHI: Efficient LLM Inference by Piggybacking Decodes with Chunked Prefills" (2023)](https://arxiv.org/abs/2308.16369) — prefill is the phase prompt 缓存 shortcuts; this paper explains why TTFT drops dramatically on cache hit while TPOT is unaffected.
- [Leviathan et al., "Fast Inference from Transformers via Speculative Decoding" (2023)](https://arxiv.org/abs/2211.17192) — prompt 缓存 sits alongside speculative decoding, Flash Attention, and MQA/GQA as levers that bend the inference cost curve; read this for the other three.
