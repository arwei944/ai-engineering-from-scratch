# 开源模型: Architecture Walkthroughs
...
**Prerequisites:** Phase 10, Lessons 04, 05, 12 (预训练, 缩放, Inference)
**Time:** ~45 分钟
...
This lesson is a diff. 对于 each major open model family, we list exactly what changed from GPT-2, why, and what it cost. 当 you are done you can read a fresh model card and mentally translate it back to the GPT-2 baseline.
...
对于 any model above ~13B parameters, GQA or MLA is effectively mandatory. Full MHA at scale is a KV cache disaster.
...
对于 Llama 3 8B at batch 1, seq 8192, BF16, 32 layers, hidden 4096: roughly 8 GB just for activations with checkpointing, 40 GB without. This is why flash-attention and ring-attention matter -- they rewrite the attention computation so activations fit.

### KV Cache budget

对于 inference at max context:
...
| Active params | "What runs per token" | 对于 MoE models, the parameter count that sees a forward pass per token (much smaller than total params) — governs per-token FLOPs |
...
- [Su et al., 2021 -- "Ro对于mer: Enhanced Transformer with Rotary Position Embedding"](https://arxiv.org/abs/2104.09864) -- the RoPE paper
