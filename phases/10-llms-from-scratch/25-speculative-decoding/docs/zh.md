...
Autoregressive generation looks inherently serial: `x_{t+1} = sample(p(· | x_{1:t}))`. 但是 there is a concurrency opportunity. 如果 you had a cheap predictor that said "the next 4 tokens are probably [a, b, c, d]" you could verify all 5 positions in a **single forward pass of the big model** and accept the longest matching prefix.

Leviathan, Kalai, Matias (2023, "Fast Inference from Transformers via 推测解码") made this exact via a clever accept/reject rule that preserves the target model's sampling distribution. The same output distribution, 2-4× faster.
...
4. 如果 any token is rejected, sample the replacement from the corrected distribution and stop. Otherwise sample one bonus token from `p(· | x_1...x_K)`.

如果 the draft matches the target perfectly, you get K+1 tokens per target-forward. 如果 the draft is wrong at position 1, you get only 1 token.
...
where `(p - q)+` denotes the positive part of the pointwise difference. 当 the draft and target agree (`p ≈ q`) acceptance is nearly 1. 当 they disagree, the residual distribution is constructed so that the overall sample is still exactly `p`.

**Greedy case.** 对于 temperature=0 sampling just check `argmax(p) == x_t`. 如果 yes, accept; if no, output `argmax(p)` and stop.

### Expected Speedup

如果 the draft model's token-level acceptance rate is `α`, the expected tokens produced per target-forward pass is:

```
E[tokens] = (1 - α^{K+1}) / (1 - α)        # K = draft length, α in [0, 1]
```

At `α = 0.8, K = 4`: `(1 - 0.8^5)/(1 - 0.8) = 3.36` tokens per forward. A single target forward costs roughly `cost_q * K + cost_p` (K draft steps plus one target verify). 如果 `cost_p >> cost_q * K` the speedup ratio is `3.36× / 1 = 3.36×` on throughput.
...
1. The draft does K serial steps, each full-stack. 但是 the draft could reuse the target's features (hidden states) from the most recent verify — the target already computed rich representations that the draft is re-deriving from scratch.
2. The draft outputs a linear chain. 如果 the draft could output a *tree* of candidates (each node multiple guesses), the target's single forward pass could verify multiple candidate paths in parallel via a tree attention mask, and pick the longest accepted branch.
...
如果 `a, b` are competing first-token candidates and `c, d, e, f` are second-token candidates, all six positions are verified in one forward pass. 输出是 the longest prefix along any accepted path.

### 当 It Wins, 当 It Doesn't
...
- [Leviathan, Kalai, Matias, 2023 — "Fast Inference from Transformers via 推测解码"](https://arxiv.org/abs/2211.17192) — the exact rejection rule and the theoretical speedup analysis
- [Chen, Borgeaud, Irving et al., 2023 — "Accelerating 大语言模型 Decoding with Speculative Sampling"](https://arxiv.org/abs/2302.01318) — concurrent speculative-sampling paper at DeepMind
...
- [Li et al., 2025 — "EAGLE-3: 缩放 up Inference Acceleration of Large Language Models via Training-Time Test"](https://arxiv.org/abs/2503.01840) — train-time test-time matching
- [Fu, Haotian, Peng et al., 2024 — "Break the Sequential Dependency of LLM Inference 使用 Lookahead Decoding"](https://arxiv.org/abs/2402.02057) — Jacobi/lookahead decoding, a speculator-free alternative
