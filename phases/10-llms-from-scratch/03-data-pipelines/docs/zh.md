...
> 模型是 a mirror. It reflects whatever data you feed it. Feed it garbage, it reflects garbage with perfect fluency.

**类型:** 实现
**Languages:** Python
**Prerequisites:** Phase 10, Lessons 01-02 (Tokenizers, Building a 分词器)
**Time:** ~90 分钟
...
Exact deduplication is easy: hash each document, remove duplicates. 但是 near-duplicates are the real problem. Two copies of the same news article with slightly different ads around it are near-duplicates. The content is 95% identical, but byte-for-byte they differ.
...
2. **MinHash:** 对于 each document's shingle set, compute k hash values. Each hash value is the minimum hash across all shingles under a different hash function. This creates a fixed-size "signature" that approximates the Jaccard similarity between any two documents.
...
4. **Verify:** 对于 each candidate pair, compute exact Jaccard similarity. Remove one copy if similarity exceeds a threshold (typically 0.8).
...
### The Chinchilla 缩放 Law

对于 a fixed compute budget C (measured in FLOPs), the optimal model size N and dataset size D follow:
...
Strip HTML, normalize whitespace, remove non-text content. 我们将 use a public domain text (Project Gutenberg) as our small corpus.
...
Compression ratio tells you how efficient the tokenizer is on this corpus. English text typically compresses to about 3-4 characters per token. 如果 you see 1.5 characters per token, your tokenizer is splitting too aggressively. 如果 you see 8+, it has learned very domain-specific merges.
...
### Compare 使用 HuggingFace Datasets
...
The HuggingFace pipeline uses Rust tokenizers under the hood and parallel processing across 4 cores. Your pure Python pipeline will be 10-50x slower. That gap is why production teams use compiled tokenizers. 算法是 the same. The implementation language is the difference.
...
| Chinchilla scaling | "Train on more data" | 对于 a fixed compute budget, optimal performance requires scaling model size and training tokens roughly equally |
