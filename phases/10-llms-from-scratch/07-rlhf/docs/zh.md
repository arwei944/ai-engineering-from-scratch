...
> SFT teaches the model to follow instructions. 但是 it doesn't teach the model which response is BETTER. Two grammatically correct, factually accurate answers can differ enormously in helpfulness. RLHF is how you encode human judgment into the model's behavior. It's what makes Claude helpful and GPT polite.
...
Both responses are factually correct. Both are grammatically sound. Both follow the instruction. 但是 Response A is clearly better. It's more concise, more informative, and better structured. A human would pick A every time.

SFT can't capture this distinction. It trains the model on "correct" responses, but it has no mechanism for saying "this response is better than that one." It treats every training example as equally good. 如果 both A and B appeared in the SFT dataset, the model would learn from both equally.
...
Training data is human preference pairs. 对于 each prompt, annotators see two responses and pick the better one. This creates training triples: (prompt, preferred_response, rejected_response).
...
**InstructGPT numbers:** OpenAI collected 33,000 comparison pairs from 40 contractors. Each comparison took about 5 分钟. That's 2,750 小时 of human labor for the reward model training data.
...
Why the KL penalty? 使用out it, the model finds degenerate solutions. The reward model is trained on a finite dataset of human preferences. It has blind spots. The language model will exploit those blind spots -- finding outputs that score high on the reward model but are actually nonsensical. Classic examples:
...
The clipping prevents catastrophic updates. 如果 a single response gets an unusually high reward, the unclipped ratio could be very large, causing the model to dramatically shift toward that response. Clipping caps the update, maintaining training stability.
...
| 对于mat gaming | Model uses bullet points and headers excessively | 对于matted responses looked more "polished" to annotators |
...
|------|----------------|----------------------|
| RLHF | "Training with human feedback" | 基于人类反馈的强化学习: a three-stage pipeline (SFT, reward model, PPO) that optimizes language model outputs using human preference signals |
...
| Reward hacking | "Gaming the reward" | 当 the policy finds degenerate high-reward outputs by exploiting weaknesses in the reward model instead of genuinely improving |
...
- [Bai et al., 2022 -- "Training a Helpful and Harmless Assistant with 基于人类反馈的强化学习"](https://arxiv.org/abs/2204.05862) -- Anthropic's RLHF paper with detailed analysis of reward hacking and KL penalty
