...
Your BPE tokenizer from Lesson 01 works on English text. Now throw Japanese at it. 或者 emoji. 或者 Python code with mixed tabs and spaces.
...
|-------|-------------|----------------|
| Normalize | NFKC Unicode, lowercase optional, strip accents optional | "fi" ligature (U+FB01) becomes "fi" (two chars). 使用out this, same word gets different tokens. |
...
Lesson 01's tokenizer operated on UTF-8 bytes. That was the right call. 但是 we skipped something important: what happens when those bytes are not valid UTF-8?
...
当 you send messages to a chat model, the API accepts a list of messages:
...
对于 perspective: tokenizing 15 trillion tokens for Llama 3 pre-training at 1 million tokens per second (fast Python) would take 174 days. At 100 million tokens per second (Rust), it takes 1.7 days.
...
The `regex` module supports Unicode property escapes (`\p{L}` for letters, `\p{N}` for numbers). The standard library `re` module does not, so we fall back to ASCII character classes. 对于 production multilingual tokenizers, install `regex`.
...
|------|----------------|----------------------|
| Byte-level BPE | "分词器 that works on bytes" | BPE with a base vocabulary of 256 byte values -- handles any input without unknown tokens |
...
| Merge table | "The vocabulary" | 或者dered list of byte-pair merges learned during training -- this IS the tokenizer's learned knowledge |
