# 梯度检查点 and Activation Recomputation

> Backprop keeps every intermediate activation. At 70B parameters and 128K context that is 3 TB of activations per rank. Checkpointing trades FLOPs for memory: recompute instead of save. 问题是 which segments to drop, and the answer is not "all of them."

**类型:** 实现
**Languages:** Python (with numpy, optional torch)
**Prerequisites:** Phase 10 Lesson 04 (Pre-Training Mini-GPT), Phase 10 Lesson 05 (缩放 & Distributed)
**Time:** ~70 分钟

## The Problem

Training a transformer stores, for each layer, the inputs to every op that is differentiated in backward: the attention inputs, the Q/K/V projections, the softmax output, the FFN inputs, the norm outputs, and the residual stream. 对于 a layer with hidden size `d`, sequence length `L`, batch `B`, this is on the order of `12 * B * L * d` floats per layer.

对于 `d=8192, L=8192, B=1`, that's 800 MB/layer in BF16. A 64-layer model is 51 GB of activations — and that's before you multiply by microbatch size, before you add attention-softmax intermediates (`L^2` per head), and before you factor tensor-parallel partial copies.
...
Done naively, checkpointing costs roughly 33% more forward-pass FLOPs per step. Done well — selective checkpointing per the "smart selection" of Korthikanti et al. — you save 5x memory for under 5% FLOP overhead. 并且 with FP8 matmuls, FSDP offload, and expert-parallel MoE this really matters: you can't afford either the memory or the wasted compute.
...
Split the network into `N` segments. During forward, store only the *input* to each segment. 当 backward needs intermediates, rerun the segment's forward pass to materialize them, then differentiate.

示例： 32-layer transformer split into 32 segments of 1 layer each.
...
This is the original Chen et al. 2016 recipe: one checkpoint every `sqrt(L)` layers to balance memory and compute. 对于 L=64, that's 8 checkpoints.

### Selective Checkpointing (Korthikanti 2022)

Not all activations cost the same. The attention softmax output is `B*L*L*heads` and grows *quadratically* with sequence length. The FFN hidden activation is `B*L*4d` and grows linearly. 对于 long sequences the softmax dominates.
...
使用 selective checkpointing you recompute only the attention kernel, not the whole layer:
...
Activation volume per layer: `A`. 对于 `L` layers, total activation memory: `L * A`.
...
### Step 1: A Toy Model 使用 Segments
...
| sqrt-L rule | "The classical optimum" | 对于 uniform-cost layers, optimal checkpoint spacing is sqrt(L) layers |
