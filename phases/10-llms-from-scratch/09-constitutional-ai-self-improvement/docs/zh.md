...
- Implement the 宪法AI two-stage loop: self-critique plus self-revision, then preference training on the revised pairs
...
The 2022 宪法AI paper asked a simple question. What if the model generates the preference labels itself? Give it a list of written principles -- the "constitution" -- and have it critique its own responses. The critiques become the training signal.
...
These two loops -- 宪法AI for subjective behavior and rule-based RL for verifiable behavior -- are the dominant alignment recipes of 2026. The human preference budget that used to go into RLHF now pays for a much smaller step: picking the constitution and picking the reward rules.
...
**Stage 1: Supervised Learning from AI Feedback (SL-CAI).** Start with an SFT model that is helpful but possibly harmful. Prompt it with potentially harmful requests. 对于 each response, ask the *same model* to critique its response against a constitutional principle, then revise. Fine-tune on the revised responses. 数据集是 (prompt, revised_response) pairs.
...
GRPO throws out the value function. 对于 each prompt, it samples a group of G responses (typically G=16 or 64). The reward for each response is computed, then normalized within the group:

```
A_i = (r_i - mean(r_1, ..., r_G)) / std(r_1, ..., r_G)
```

优势是 the z-score of the response's reward relative to its siblings. No value function. The group acts as its own baseline.
...
- **对于matting**: a regex decides whether the answer is in the required XML tag.
...
The code implements three things in pure Python + numpy. A 宪法AI self-critique loop. A rule-based reward checker for simple arithmetic. A minimal GRPO trainer that runs on a tiny language model from Lesson 04.

### Step 1: The Constitution

A list of principles. In production, each line would be richer and category-tagged. 对于 the lesson, keep it short.
...
The revise function is a stand-in. 使用 a real LLM it would be a second prompt: "Given the critique, rewrite the response."

### Step 3: Rule-Based Rewards

对于 verifiable tasks, replace the critic entirely. This checker grades arithmetic answers.
...
如果 every sample in the group has the same reward, the advantage is zero and no gradient signal flows. This is a feature. It tells you the prompt is either trivially solved or impossibly hard for the current policy, and the step should skip it.
...
5. Build a process reward scorer for a two-step arithmetic problem. Given "什么是 (3+4)*5?", the model must show the intermediate 3+4=7 step. Grade the intermediate step separately from the final answer and compare PRM-weighted GRPO to pure ORM-weighted GRPO over 10 rounds.

## Key Terms

| Term | What people say | What it actually means |
|------|----------------|----------------------|
| 宪法AI | "The model aligns itself" | A two-stage pipeline (self-critique + RLAIF) that replaces most human preference labels with model self-judgments against a written constitution |
...
- [Bai et al., 2022 -- "宪法AI: Harmlessness from AI Feedback"](https://arxiv.org/abs/2212.08073) -- Anthropic's original CAI paper with the two-stage SL-CAI + RLAIF pipeline
...
- [Lightman et al., 2023 -- "让我们 Verify Step by Step"](https://arxiv.org/abs/2305.20050) -- OpenAI's PRM800K and the case for process reward models
