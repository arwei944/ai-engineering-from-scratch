...
对于 a model with hidden `h` and vocabulary `V`:
...
Total extra per module: `~14h^2`. 对于 DeepSeek-V3's `h = 7168`, D = 1 module: `~14 * 7168^2 = ~720M` parameters on paper. DeepSeek-V3 reports 14B — the difference is mostly expert layers being MoE in the MTP module too.
...
|-----------|---------|------------------|
| 当 trained | Post-pre-training | During pre-training |
...
当 to use MTP in a new pre-training run:
...
- 微调 an existing pre-trained dense model. The MTP module is not trained.
...
5. Use the trained MTP module as an EAGLE-style draft: call module k to propose `t_{i+k}` at inference. Measure the acceptance rate of these draft tokens against the main model's predictions on a held-out sequence. 如果 you hit 50%+ on the toy, you have reproduced the empirical MTP-as-draft property.
...
- [Leviathan et al. — Fast Inference from Transformers via 推测解码 (arXiv:2211.17192)](https://arxiv.org/abs/2211.17192) — the speculative-decoding framework MTP fits into
