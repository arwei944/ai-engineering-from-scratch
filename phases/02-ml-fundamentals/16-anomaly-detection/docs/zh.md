...
- Implement Z-score, IQR, and Isolation Forest 异常检测 methods from scratch
- Distinguish between point, contextual, and collective anomalies and select the appropriate detection method for each
- Explain why 异常检测 is framed as modeling normal data rather than classifying anomalies
- Compare unsupervised 异常检测 with supervised classification and evaluate the tradeoff between novel anomaly coverage and precision
...
In standard classification, you have labels for both classes. In 异常检测, you typically have one of three situations:
...
The key insight: 异常检测 is fundamentally different from classification. You are modeling the distribution of normal data, not the decision boundary between two classes.
...
- **Extreme 类别不平衡.** With 0.1% anomalies, predicting "normal" for everything gives 99.9% accuracy. Accuracy is useless.
...
The 流水线 is never "done." Data distributions shift, new anomaly types emerge, and thresholds need adjustment. Treat 异常检测 as a living system, not a one-time model.
...
For real-time 异常检测 in production:
...
4. **Streaming 异常检测.** Modify the Z-score detector to work in a streaming setting: update the running mean and variance as new points arrive (Welford's online algorithm). Compare to batch Z-score on the same data.
...
| Precision@k | "Of the top k flags, how many are real" | Precision computed on only the k most suspicious points, useful for imbalanced 异常检测 |
| AUPRC | "Area under precision-recall curve" | A metric that summarizes precision-recall performance across all thresholds, better than AUROC for 不平衡数据 |
...
- [Chandola et al., 异常检测: A Survey (2009)](https://dl.acm.org/doi/10.1145/1541880.1541882) -- comprehensive survey of 异常检测 methods
- [Goldstein and Uchida, A Comparative Evaluation of Unsupervised 异常检测 Algorithms (2016)](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0152173) -- empirical comparison of 10 methods on real datasets
