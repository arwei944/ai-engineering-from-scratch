...
- Decompose a 时间序列 into trend, seasonality, and residual components and test for stationarity
- Implement lag features and rolling statistics to convert a 时间序列 into a supervised learning problem
- Build a walk-forward validation framework that prevents future data from leaking into training
- Explain why random train/test splits are invalid for 时间序列 and demonstrate the performance gap versus proper temporal splits
...
This lesson covers the fundamentals: what makes time data different, how to evaluate models honestly, and how to turn a 时间序列 into features that standard ML models can consume.
...
In standard ML, samples are interchangeable. Shuffling them changes nothing. In 时间序列, order is everything. Shuffling destroys the signal.
...
A 时间序列 is stationary if its statistical properties (mean, variance, autocorrelation) do not change over time. Most forecasting methods assume stationarity.
...
### Lag Features: Turning 时间序列 into Supervised Learning
...
**How many lags?** Use the autocorrelation function. If ACF is significant up to lag 10, use at least 10 lags. If there is weekly seasonality, include lag 7 (and possibly 14). More lags give the model more history but also more features to fit, increasing the risk of 过拟合.

**The target alignment trap.** When creating lag features, the target must be the value at time t, and all features must use values at time t-1 or earlier. If you accidentally include the value at time t as a feature, you have a perfect predictor -- and a completely useless model. This is the most common bug in 时间序列 特征工程.

### Walk-Forward Validation

This is the most important concept in this lesson. Standard k-fold cross-validation randomly assigns samples to train and test. For 时间序列, this leaks future information.
...
ARIMA is the classical 时间序列 model. It has three components:
...
| 过拟合 to seasonality | Model memorizes calendar patterns | Hold out a full seasonal cycle in the test set |
...
| ARIMA | "The classic 时间序列 model" | AutoRegressive Integrated Moving Average: combines past values (AR), differencing (I), and past errors (MA) |
...
- [scikit-learn 时间序列 Split](https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.TimeSeriesSplit.html) -- sklearn's walk-forward splitter
