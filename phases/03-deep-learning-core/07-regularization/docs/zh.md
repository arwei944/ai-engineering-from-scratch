...
> Your model gets 99% on training data and 60% on test data. It memorized instead of learning. 正则化 is the tax you impose on complexity to force generalization.

**类型:** 实现
**语言:** Python
**Prerequisites:** Lesson 03.06 (优化器s)
**Time:** ~75 minutes

## 学习目标

- Implement dropout with inverted scaling, L2 权重衰减, 批归一化, layer normalization, and RMSNorm from scratch
- Measure the train-test accuracy gap and diagnose overfitting using 正则化 experiments
- Explain why transformers use LayerNorm instead of BatchNorm and why modern LLMs prefer RMSNorm
- Apply the correct combination of 正则化 techniques based on the severity of overfitting

## The Problem

A 神经网络 with enough parameters can memorize any 数据集. This is not a hypothetical -- Zhang et al. (2017) proved it by training standard networks on ImageNet with random labels. The networks reached near-zero training loss on completely random label assignments. They memorized a million random input-output pairs with no pattern to learn. Training loss was perfect. Test accuracy was zero.

This is the overfitting problem, and it gets worse as models get larger. GPT-3 has 175 billion parameters. The training set has about 500 billion tokens. With that many parameters, the model has enough capacity to memorize significant chunks of the training data verbatim. Without 正则化, it would just regurgitate training examples instead of learning generalizable patterns.

The gap between training performance and test performance is the overfitting gap. Every technique in this lesson attacks that gap from a different angle. Dropout forces the network to not rely on any single neuron. Weight decay prevents any single weight from growing too large. Batch normalization smooths the loss landscape so the 优化器 finds flatter, more generalizable minima. Layer normalization does the same thing but works where 批归一化 fails (small batches, variable-length sequences). RMSNorm does it 10% faster by dropping the mean calculation. Each technique is simple. Together, they're the difference between a model that memorizes and one that generalizes.

## The Concept

### The Overfitting Spectrum

Every model sits somewhere on a spectrum from underfitting (too simple to capture the pattern) to overfitting (so complex it captures noise). The sweet spot is in between, and 正则化 pushes models toward it from the overfit side.
...
The simplest 正则化 technique with the most elegant interpretation. During training, randomly set each neuron's output to zero with probability p.
...
Default rates: p = 0.1 for transformers, p = 0.5 for MLPs, p = 0.2-0.3 for CNNs. Higher dropout = stronger 正则化 = more underfitting risk.
...
The gradient of the 正则化 term is lambda * w. This means at every step, each weight is shrunk toward zero by a fraction proportional to its magnitude. Large weights get penalized more. The model is pushed toward solutions where no single weight dominates.
...
As discussed in lesson 06: 权重衰减 and L2 正则化 are equivalent in SGD but not in Adam. Always use AdamW (decoupled 权重衰减) when training with Adam.
...
The effect is identical to 正则化: it increases the effective size of the training set, making it harder for the model to memorize specific examples. A model that only sees each image once in its original form can memorize it. A model that sees 50 augmented versions of each image is forced to learn the invariant structure.
...
```mermaid
flowchart TD
    Gap{"Train-test<br/>accuracy gap?"} -->|"> 10%"| Heavy["Heavy 正则化"]
    Gap -->|"5-10%"| Medium["Moderate 正则化"]
    Gap -->|"< 5%"| Light["Light 正则化"]
...
### Step 6: Training With and Without 正则化
...
PyTorch provides all normalization and 正则化 as 模块s:
...
- `outputs/prompt-正则化-advisor.md` -- a prompt that diagnoses overfitting and recommends the right 正则化 strategy

## Exercises

1. Implement spatial dropout for 2D data: instead of dropping individual neurons, drop entire feature channels. Simulate this by treating groups of consecutive features as channels and dropping whole groups. Compare the train-test gap to standard dropout on the circle 数据集 with hidden_size=32.
...
3. Add a BatchNorm layer between the hidden layer and the activation in your circle-数据集 network. Train with and without BatchNorm at 学习率s 0.01, 0.05, and 0.1. BatchNorm should allow stable training at higher 学习率s where the vanilla network diverges.
...
| 正则化 | "Preventing overfitting" | Any technique that constrains model complexity to improve generalization: dropout, 权重衰减, normalization, augmentation |
...
| Data augmentation | "More data from less" | Transforming training inputs (flip, crop, noise) to increase effective 数据集 size and force invariance learning |
| Generalization gap | "Train-test split" | The difference between training and test performance; 正则化 aims to minimize this gap |

## Further Reading

- Srivastava et al., "Dropout: A Simple Way to Prevent 神经网络s from Overfitting" (2014) -- the original dropout paper with the ensemble interpretation and extensive experiments
- Ioffe & Szegedy, "批归一化: Accelerating Deep Network Training by Reducing Internal Covariate Shift" (2015) -- introduced BatchNorm and its training procedure, one of the most cited deep learning papers
...
- Zhang et al., "Understanding 深度学习 Requires Rethinking Generalization" (2017) -- the landmark paper showing 神经网络s can memorize random labels, challenging traditional views of generalization
