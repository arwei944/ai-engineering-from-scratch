...
Language models work on discrete tokens. Audio is continuous. 如果 you want an LLM-style model for speech / music — MusicGen, Moshi, Sesame CSM, VibeVoice, 或者pheus — you first need a **neural audio codec**: a learned encoder that discretizes audio into a small vocabulary of tokens, and a matching decoder that reconstructs the waveform.
...
2. **Semantic-first codecs** — Mimi (Kyutai), SpeechTokenizer. 对于ce the first codebook to encode linguistic / phonetic content (often by distilling from WavLM). Subsequent codebooks are acoustic detail.
...
**SNAC (Hubert Siuzdak, 2024).** Multi-scale RVQ — the coarse codebooks operate at a lower frame rate than fine ones. Effectively models audio hierarchically: a coarse "sketch" at ~12 Hz plus detail at 50 Hz. Used by 或者pheus-3B because the hierarchical structure maps well onto LM-based generation.
...
| 编解码器 | Frame rate | 1 s = N frames | Good for |
|-------|-----------|----------------|---------|
...
对于 a 10 s speech clip at Mimi's 12.5 Hz × 8 codebooks:
...
| 流式 full-duplex speech | Mimi (12.5 Hz) |
| Sound-effect library with text | EnCodec + T5 condition |
| Fine-grained audio editing | DAC + inpainting |

Rule of thumb: **if you're building a generative model, start with Mimi or SNAC. 如果 you're building a compression pipeline, use Opus.**
...
- **使用 reconstruction quality as the only metric.** A codec can have great reconstruction but be useless for LM-based generation if the semantic structure is bad.
...
| Frame rate | 编解码器 speed | How many token-frames per second. Lower = faster LM. |
...
- [Kumar et al. (2023). Descript Audio 编解码器 (DAC)](https://arxiv.org/abs/2306.06546) — highest-fidelity open.
