# 流式 Speech-to-Speech — Moshi, Hibiki, and Full-Duplex Dialogue
...
答案是 **full-duplex speech-to-speech**. Theoretical latency 160 ms (80 ms Mimi frame + 80 ms acoustic delay). Practical latency 200 ms on a single L4 GPU. That's half what a best-in-class pipelined voice agent achieves.
...
**The depth transformer.** 使用in a frame, the 8 codebooks are not predicted in parallel — they have inter-codebook dependencies. A small 2-layer "depth transformer" predicts them sequentially within 80 ms. This is the standard factorization for AR codec LMs (also used by VALL-E, VibeVoice).
...
Four language pairs supported initially; can be adapted to a new language with ≈1000 小时.
...
Sesame CSM (2025) uses a similar idea — a Llama-3 backbone with a Mimi codec head. 但是 CSM is single-directional (takes context + text, produces speech) rather than full-duplex. It's the best "voice presence" TTS on the market; not quite the same as Moshi's full-duplex capability.
...
| 流式 S2S | Audio → audio live | Chunk-by-chunk translation/dialogue, no pipeline stages. |
