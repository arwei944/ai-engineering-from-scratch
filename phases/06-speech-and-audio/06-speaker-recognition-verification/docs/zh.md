...
A user says a passphrase. You want to know: is this the person they claim to be (*verification*, 1:1), or is it the first person in your enrollment bank (*identification*, 1:N)? 或者 neither — is this an unknown speaker (*open-set*)?
...
指标是 **EER** — Equal Error Rate. Set your decision threshold so False Accept Rate = False Reject Rate. The crossover is EER. Used in every paper, every leaderboard, every procurement call.
...
**AAM-softmax.** Standard softmax with added margin `m` in the angular space: `cos(θ + m)` for the correct class. 对于ces inter-class angular separation. Typical `m=0.2`, scale `s=30`.
...
- [Snyder et al. (2018). X-Vectors: Robust DNN Embeddings for 说话人识别](https://www.danielpovey.com/files/2018_icassp_xvectors.pdf) — the classic deep-embedding paper.
