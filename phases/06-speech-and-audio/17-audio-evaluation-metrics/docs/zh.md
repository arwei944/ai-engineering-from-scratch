...
Every audio task has multiple metrics, each measuring a different axis. 使用 the wrong metric is how you ship a model that looks great on your dashboard and terribly in production. The 2026 canonical list:
...
| 流式 S2S | latency P50/P95 | WER · MOS |
...
**WER (词错误率).** `(S + D + I) / N`. Lowercase, strip punctuation, normalize numbers before scoring. Use `jiwer` or OpenAI's `whisper_normalizer`. &lt; 5% = human-parity read speech.

**CER (字错误率).** Same formula, character-level. Used for tone languages (Mandarin, Cantonese) where word segmentation is ambiguous.
...
**MOS (平均意见得分).** 1-5 human rating. Gold standard but slow. Collect 20+ listeners per sample, 100+ samples per model.
...
**SECS (Speaker Encoder Cosine Similarity).** 对于 voice cloning. ECAPA embedding cosine between reference and cloned output. &gt; 0.75 = recognizable clone.
...
**MMAU (Massive Multi-Audio 理解).** 10k audio-QA pairs.
...
| CER | Character WER | 对于 tone languages or char-level systems. |
