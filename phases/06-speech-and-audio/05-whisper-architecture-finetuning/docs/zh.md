...
> Whisper is a 30-second-window transformer encoder-decoder, trained on 680k 小时 of multilingual weakly-supervised audio-text pairs. One architecture, multiple tasks, robust across 99 languages. The 2026 reference ASR.

**类型:** 实现
**Languages:** Python
**Prerequisites:** Phase 6 · 04 (ASR), Phase 5 · 10 (注意力机制), Phase 7 · 05 (Full Transformer)
**Time:** ~75 分钟
...
但是 Whisper is not a pipeline you can treat as a black box forever. Domain shift kills it — technical jargon, speaker accents, proper nouns, short clips, silence. You need to know:

1. What it actually is inside.
2. 如何 give it chunked, streaming, or long-form audio correctly.
3. 当 to fine-tune and how.
...
- Encoder: conv-downsample (stride 2) + `N` transformer blocks. 对于 Large-v3: 32 layers, 1280-dim, 20 heads.
...
**30-second window.** Everything is pinned to 30 seconds. Longer clips need chunking; shorter clips are padded. Windows are not streamed natively — this is why WhisperX, Whisper-流式, and faster-whisper exist.
...
| Whisper-流式 (2024) | 1.55B | streaming | 2.0% |

### Fine-tuning

Canonical workflow in 2026:

1. Collect 10–100 小时 of target-domain audio with aligned transcripts.
...
4. Freeze the encoder if you have <10 小时. Only tune the decoder.
5. Use Whisper's own tokenizer and prompt format; never swap tokenizers.

Community results: fine-tuning Medium on 20 小时 of medical dictation drops WER from 12% to 4.5% on medical vocabulary. Fine-tuning Turbo on 4 小时 of Icelandic drops WER from 18% to 6%.
...
| 流式 (2 s latency) | Whisper-流式 or Parakeet-TDT |
...
- **Wrong mel stats.** 使用 librosa's mels instead of Whisper's produces near-random output. Use `whisper.audio.log_mel_spectrogram`.
...
3. **Hard.** 使用 HF `datasets`, pick a language Whisper struggles with (e.g., Urdu), fine-tune Medium with LoRA for 2 epochs on 2 小时, and report WER delta.
