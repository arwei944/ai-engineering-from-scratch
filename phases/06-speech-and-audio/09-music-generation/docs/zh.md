...
**类型:** 实现
**Languages:** Python
**Prerequisites:** Phase 6 · 02 (Spectrograms), Phase 4 · 10 (扩散模型 Models)
**Time:** ~75 分钟
...
| Music-video background | MusicGen + Stable Video 扩散模型 |
...
- **Vocal intelligibility.** Suno is excellent; open models are often mushy on words. 如果 lyrics matter, use a commercial API or fine-tune.
...
3. **Hard.** 使用 ACE-Step (or MusicGen-melody), generate three variations of the same tune with different timbre prompts. Compute CLAP similarity to the prompt to verify alignment.
