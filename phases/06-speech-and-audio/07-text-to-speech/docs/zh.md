# 文本转语音 (TTS) — From Tacotron to F5 and Kokoro
...
In 2026 the acoustic + vocoder split blurs with end-to-end diffusion and flow-matching models. 但是 the mental model of three parts still holds for debugging.
...
**F5-TTS (2024).** 扩散模型 transformer over flow matching. Natural prosody, zero-shot voice cloning with 5 seconds of reference audio. Top of the 2026 open-source TTS leaderboards. 335M params.
...
- **MOS (平均意见得分).** 1–5 scale, crowd-sourced. Still the gold standard; painfully slow.
...
- **CER (字错误率) via ASR.** Run TTS output through Whisper, compute CER against the input text. Proxy for intelligibility.
...
| Situation | Pick |
|-----------|------|
| 实时 English voice assistant | Kokoro (CPU) or XTTS v2 (GPU) |
