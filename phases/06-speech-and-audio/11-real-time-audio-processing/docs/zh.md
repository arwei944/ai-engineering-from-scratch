...
> Batch pipelines process a file. 实时 pipelines process the next 20 milliseconds before the next 20 arrive. Every conversational AI, broadcast studio, and telephony bot lives and dies by this latency budget.
...
![流式 audio pipeline with ring buffer, VAD gate, interruption](../assets/real-time.svg)

**Frame / chunk / window.** 实时 audio flows as fixed-size blocks. Common choice: 20 ms (320 samples at 16 kHz). Everything downstream must keep up with this cadence.
...
**VAD (语音活动检测).** Gates downstream work when nobody is speaking. Silero VAD 4.0 (2024) runs <1 ms per 30 ms frame on CPU. `webrtcvad` is the older alternative.

**流式 ASR.** Models that emit partial transcripts as audio arrives. Parakeet-CTC-0.6B in streaming mode (NeMo, 2024) does 2–5% WER at 320 ms latency. Whisper-流式 (Macháček et al., 2023) chunks Whisper for near-streaming at ~2 s latency.

**Interruption.** 当 the user speaks while the assistant is talking, you must (a) detect the barge-in, (b) stop the TTS, (c) discard the remaining LLM output. All within 100 ms, or the user perceives deaf assistant.
...
- **Echo cancellation.** 使用out AEC, TTS output re-enters the mic and triggers ASR on the bot's own voice. WebRTC AEC3 is the open-source default.
...
| VAD | Silero VAD 4.0 |
| 流式 ASR | Parakeet-CTC-0.6B or Whisper-流式 |
| LLM first-token | Groq, Cerebras, vLLM-streaming |
| 流式 TTS | Kokoro or ElevenLabs Turbo v2.5 |
...
2. **Medium.** 使用 `sounddevice`, build a passthrough loop that processes your mic in 20 ms frames and prints VAD state at each frame.
...
| 流式 ASR | 实时 STT | Emits partial text as audio arrives; bounded lookahead. |
...
- [Macháček et al. (2023). Whisper-流式](https://arxiv.org/abs/2307.14743) — chunked near-streaming Whisper.
