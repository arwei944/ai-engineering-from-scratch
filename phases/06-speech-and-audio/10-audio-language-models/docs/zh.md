...
| GAMA | Llama | AST + Q-对于mer | text | Apache-2.0 |
...
- Speaker-attributed reasoning over long conversations (degrades past 10 分钟).
- Multi-audio comparison (22-26% is barely above random).
- 实时 streaming reasoning (most are offline batch inference).
...
- **Over-trust on multi-audio.** 如果 your task needs "which clip has X," random-chance-level performance is real.
- **Long-audio degradation.** Past 10 分钟, most models' speaker attribution breaks. Diarize first (Lesson 6), then summarize.
