# Evaluation — FID, CLIP分数, Human Preference

> Every generative model leaderboard cites FID, CLIP score, and a win rate from a human-preference arena. Each number has a failure mode a determined researcher can game. 如果 you do not know the failure modes, you cannot tell a real improvement from a gaming run.

**类型:** 实现
**Languages:** Python
**Prerequisites:** Phase 8 · 01 (分类体系), Phase 2 · 04 (Evaluation Metrics)
**Time:** ~45 分钟
...
- **FID (弗雷歇 inception 距离).** A distance between two distributions — real and generated — in an Inception network's feature space. Lower is better.
...
Radford et al. (2021). 对于 a generated image + prompt:
...
- **Elo bias from prompt overlap.** 如果 both models saw a benchmark prompt during training, Elo is meaningless. Use held-out prompt sets.
...
|------|-----------------|-----------------------|
| FID | "弗雷歇 inception 距离" | Fréchet distance of Gaussian fits to real vs gen Inception features. |
...
Running FID on 10k samples means generating 10k images. 对于 a 50-step SDXL base at 1024² on a single L4, that is ~11 小时 of single-request inference. Evaluation budgets are real, and the framing is exactly the offline-inference scenario (maximize throughput, ignore TTFT):
...
对于 CI / regression gates: run FID + CLIP score on a 500-sample subset per PR (~30 min); run full 10k FID + HPSv2 + Elo nightly.
