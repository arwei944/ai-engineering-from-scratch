...
**类型:** 实现
**Languages:** Python
**Prerequisites:** Phase 3 · 02 (Backprop), Phase 3 · 07 (CNNs), Phase 8 · 01 (分类体系)
**Time:** ~75 分钟
...
![自编码器 vs VAE: the reparameterization trick](../assets/vae.svg)

**自编码器.** `z = encoder(x)`, `x̂ = decoder(z)`, loss = `||x - x̂||²`. Code space unstructured.

**VAE encoder.** Outputs two vectors: `μ(x)` and `log σ²(x)`. These define `q(z|x) = N(μ, diag(σ²))`.

**Reparameterization trick.** 采样 from `q(z|x)` is not differentiable. Rewrite the sample as `z = μ + σ·ε` where `ε ~ N(0, I)`. Now `z` is a deterministic function of `(μ, σ)` plus a non-parameter noise — gradients flow through `μ` and `σ`.
...
**采样.** At inference: draw `z ~ N(0, I)`, forward through decoder. One forward pass — no iterative sampling like diffusion.

## 从零实现

`code/main.py` implements a tiny VAE without numpy or torch. Input is 8-dimensional synthetic data drawn from a 2-component Gaussian mixture in 8-D. 编码器 and decoder are single hidden-layer MLPs. We implement tanh activation, forward pass, loss, and a hand-written backward pass. Not production — pedagogy.
...
- **Blurry samples.** The Gaussian decoder likelihood implies MSE reconstruction, which is Bayes-optimal for L2 (the mean) — the mean of a set of plausible digits is a fuzzy digit. Fix: discrete decoder (VQ-VAE, NVAE), or use the VAE only as an encoder and stack diffusion on the latents (this is what Stable 扩散模型 does).
- **β too large, too early.** See posterior collapse. Start at β≈0.01 and ramp.
- **Latent dim too small.** 16-D works for MNIST, 256-D for ImageNet 256², 2048-D for ImageNet 1024². Stable 扩散模型's VAE compresses 512×512×3 → 64×64×4 (32x downsample factor in spatial area, 32x in channels).
...
| Situation | Pick |
|-----------|------|
| Image-latent encoder for diffusion | Stable 扩散模型 VAE (`sd-vae-ft-ema`) or Flux VAE |
...
|------|-----------------|-----------------------|
| 自编码器 | Encode-decode network | `x → z → x̂`, learn MSE. Not generative. |
| VAE | AE with a sampler | 编码器 outputs a distribution, KL penalty shapes code space. |
...
| Posterior collapse | "KL term wins" | 编码器 ignores `x`, outputs the prior; decoder must hallucinate. |
...
- [Vahdat & Kautz (2021). NVAE: A Deep Hierarchical 变分自编码器](https://arxiv.org/abs/2007.03898) — state-of-the-art image VAE.
- [Rombach et al. (2022). High-Resolution Image Synthesis with Latent 扩散模型 Models](https://arxiv.org/abs/2112.10752) — Stable 扩散模型; VAE as encoder.
