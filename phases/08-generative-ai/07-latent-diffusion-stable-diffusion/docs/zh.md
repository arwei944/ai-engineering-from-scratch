...
This is the Stable 扩散模型 recipe. SD 1.x / 2.x used an 860M U-Net over `64×64×4` latents, SDXL used a 2.6B U-Net over `128×128×4`, SD3 swapped the U-Net for a 扩散模型 Transformer (DiT) with flow matching. Flux.1-dev (Black 对于est Labs, 2024) ships a 12B-param DiT-MMDiT. All run on the same two-stage substrate.
...
A real VAE has trained weights. 对于 pedagogy, this linear map is enough to show that diffusion operates on `z` without caring about the original data space.
...
This is the only substantive difference between a class-conditional diffusion model and Stable 扩散模型.

## Pitfalls

- **VAE-scale mismatch.** SD 1.x VAEs have a scaling constant (`scaling_factor ≈ 0.18215`) applied after encoding. 对于getting this makes the U-Net train on latents with wildly wrong variance. Every checkpoint ships one.
...
3. **Hard.** Set up a real Stable 扩散模型 inference with diffusers: load `sdxl-base`, run 30 Euler steps with CFG=7, time it. Now switch to `sdxl-turbo` with 4 steps and CFG=0. Same subject, different quality — describe what changed and why.
...
| Second stage | "The U-Net" | 扩散模型 model over the latent space. |
| CFG | "引导 scale" | `(1+w)·ε_cond - w·ε_uncond`; tunes conditioning strength. |
...
| DiT | "扩散模型 Transformer" | Replace U-Net with a transformer over latent patches; scales better. |
...
2. **4-bit quantization via bitsandbytes.** `Bits并且BytesConfig(load_in_4bit=True, bnb_4bit_compute_dtype=torch.bfloat16)` on both the T5 encoder and the DiT. Cuts memory 8×, quality drop is imperceptible for text-to-image per Aritra's benchmarks (linked in the notebook).
...
The memory accounting is: `10 GB T5 / 8 = 1.25 GB` quantized, `12 B params × 0.5 bytes = ~6 GB` quantized DiT, plus activations. In stas00's terms this is the extreme-end of TP=1 inference — no model parallelism, maximum quantization. 对于 production you'd run TP=2 or TP=4 on H100s; for a single dev laptop, this is the recipe.

## 延伸阅读

- [Rombach et al. (2022). High-Resolution Image Synthesis with Latent 扩散模型 Models](https://arxiv.org/abs/2112.10752) — Stable 扩散模型.
- [Podell et al. (2023). SDXL: Improving Latent 扩散模型 Models for High-Resolution Image Synthesis](https://arxiv.org/abs/2307.01952) — SDXL.
- [Peebles & Xie (2023). Scalable 扩散模型 Models with Transformers (DiT)](https://arxiv.org/abs/2212.09748) — DiT.
- [Esser et al. (2024). Scaling 整流流 Transformers for High-Resolution Image Synthesis](https://arxiv.org/abs/2403.03206) — SD3, MMDiT.
- [Ho & Salimans (2022). Classifier-Free 扩散模型 引导](https://arxiv.org/abs/2207.12598) — CFG.
- [Labs (2024). Flux.1 — Black 对于est Labs announcement](https://blackforestlabs.ai/announcing-black-forest-labs/) — Flux.1 family.
