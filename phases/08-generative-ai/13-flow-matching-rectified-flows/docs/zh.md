...
> 扩散模型 models take 20-50 sampling steps because they walk a curved path from noise to data. Flow matching (Lipman et al., 2023) and rectified flow (Liu et al., 2022) trained straight paths. Straighter paths mean fewer steps mean faster inference. Stable 扩散模型 3, Flux.1, and AudioCraft 2 all switched to flow matching in 2024.
...
如果 you could train the model such that the path from noise to data was a *straight line*, a single Euler step from `t=1` to `t=0` would work. Flow matching builds this directly: define a straight-line interpolation from `x_1 ∼ N(0, I)` to `x_0 ∼ data`, train a vector field `v_θ(x, t)` to match its time derivative, integrate at inference.
...
当ever a paper says "faster than diffusion" in 2025-2026, it is almost always flow matching + distillation.
...
- [Liu, Gong, Liu (2022). Flow Straight and Fast: Learning to Generate and Transfer Data with 整流流](https://arxiv.org/abs/2209.03003) — rectified flow.
- [Lipman et al. (2023). 流匹配 for Generative Modeling](https://arxiv.org/abs/2210.02747) — flow matching.
- [Esser et al. (2024). Scaling 整流流 Transformers for High-Resolution Image Synthesis](https://arxiv.org/abs/2403.03206) — SD3, rectified flow at scale.
...
- [Sauer et al. (2023). Adversarial 扩散模型 Distillation (SDXL-Turbo)](https://arxiv.org/abs/2311.17042) — turbo variant.
- [Black 对于est Labs (2024). Flux.1 models](https://blackforestlabs.ai/announcing-black-forest-labs/) — flow matching in production.
