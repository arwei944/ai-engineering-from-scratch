...
**U-Net generator.** 编码器-decoder with skip connections across the bottleneck. Critical for tasks where input and output share low-level structure (edges, silhouette). 使用out the skips, high-frequency detail vanishes.
...
- **Ground-truth leakage in D.** Concatenate `(x, y)` as D input, not just `y`. 使用out this D cannot check consistency.
...
| Style transfer | 扩散模型 with IP-Adapter or LoRA; GAN methods are legacy |
| Depth → photo | ControlNet-Depth over Stable 扩散模型 |
...
|------|-----------------|-----------------------|
| 条件GAN | "GAN with labels" | G(z, c), D(x, c). Both networks see the condition. |
| Pix2Pix | "Image-to-image GAN" | Paired cGAN with U-Net G and PatchGAN D + L1 loss. |
| U-Net | "编码器-decoder with skips" | Symmetric conv network; skips preserve high-freq. |
...
当 you have paired data and a narrow task (sketch → render, semantic map → photo, day → night), Pix2Pix's one-shot inference beats diffusion by an order of magnitude on latency. The production comparison is usually:
...
Pix2Pix wins on throughput in static batches (every request is the same FLOPs). 扩散模型 wins on quality and generalization. The modern play is often to ship a Pix2Pix-style distilled model for the narrow task and a diffusion fallback for tail inputs.
...
- [Miyato & Koyama (2018). cGANs with Projection 判别器](https://arxiv.org/abs/1802.05637) — the projection D.
