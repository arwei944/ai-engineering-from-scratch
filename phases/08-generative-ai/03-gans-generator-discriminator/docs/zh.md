...
> Goodfellow's trick in 2014 was to skip density entirely. Two networks. One makes fakes. One catches them. They fight until the fakes are indistinguishable from real. It shouldn't work. It often doesn't. 当 it does, the samples are still the sharpest in the literature for narrow domains.
...
Goodfellow's idea: train a classifier `D(x)` to distinguish real images from fakes. Train a generator `G(z)` to fool `D`. The loss signal for `G` is whatever `D` currently thinks makes something look real. This signal updates as `G` improves, chasing a moving target. 如果 both networks converge, `G` has learned the data distribution without ever writing down `log p(x)`.
...
In 2026 GANs are no longer the SOTA generator (diffusion and flow matching ate that crown). 但是 StyleGAN 2/3 remain the sharpest face models ever shipped, GAN discriminators are used as *perceptual losses* in diffusion training, and adversarial training powers the fast 1-step distillations (SDXL-Turbo, SD3-Turbo, LCM) that let you ship real-time diffusion.

## The Concept

![GAN training: generator and discriminator in minimax](../assets/gan.svg)

**生成器 `G(z)`.** Maps a noise vector `z ~ N(0, I)` to a sample `x̂`. A decoder-shaped network (dense or transposed conv).

**判别器 `D(x)`.** Maps a sample to a scalar probability (or score). Real → 1, fake → 0.
...
**Why it works.** 如果 `G` perfectly matches `p_data`, then `D` cannot do better than chance and outputs 0.5 everywhere; `G` gets no more gradient. Equilibrium.
...
`code/main.py` trains a tiny GAN on 1-D data: a mixture of two Gaussians. 生成器 and discriminator are single-hidden-layer MLPs. We implement forward, backward, and the minimax loop by hand. 目标是 to see the two key failure modes (mode collapse + vanishing gradient) as they happen.
...
- **判别器 too strong.** Cut D's learning rate by 2-5x, or add instance/layer noise. 如果 D reaches >95% accuracy, G is dead.
- **生成器 memorizes a mode.** Add noise to D inputs, use a minibatch-discriminator layer, or switch to WGAN-GP.
...
| Anime / stylized faces | StyleGAN-XL or Stable 扩散模型 LoRA |
...
|------|-----------------|-----------------------|
| 生成器 | "G" | 噪声-to-sample network, `G: z → x̂`. |
| 判别器 | "D" | Classifier `D: x → [0, 1]`, real vs fake. |
| Minimax | "The game" | `min_G max_D` of a joint objective. |
| Non-saturating loss | "The fix" | Use `-log D(G(z))` for G instead of `log(1 - D(G(z)))`. |
| Mode collapse | "G memorized one thing" | 生成器 produces few distinct outputs despite diverse data. |
...
- [Sauer et al. (2023). Adversarial 扩散模型 Distillation](https://arxiv.org/abs/2311.17042) — SDXL-Turbo.
