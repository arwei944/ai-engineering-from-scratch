...
对于 any linear layer `W ∈ R^{d×d}` in the model, freeze `W` and add a low-rank delta:

```
W' = W + ΔW,  ΔW = B @ A,  A ∈ R^{r×d},  B ∈ R^{d×r}
```

with `r << d`. Rank 4-16 is standard for attention, rank 64-128 for heavy fine-tunes. Number of new parameters: `2 · d · r` instead of `d²`. 对于 SDXL attention with `d=640`, `r=16`: 20k params per adapter instead of 410k — a 20x reduction. Across the whole model: a LoRA is usually 20-200MB vs the base 5GB.
...
| Tool | What it controls | Size | 当 to use |
|------|------------------|------|-------------|
...
- **ControlNet weight conflict.** 使用 a Pose ControlNet at weight 1.0 and a Depth ControlNet at weight 1.0 usually overshoots. Sum of weights ≈ 1.0 is a safe default.
...
| Background replace | ControlNet-Seg + 图像修复 (Lesson 09) |
...
2. **Medium.** Train two separate LoRAs on two target transforms. Load them together and show their additive interaction. 当 does the interaction break linearity?
...
- **Quantized LoRAs too.** 如果 you quantized the base (see Lesson 07, Flux on 8GB), the LoRA delta also quantizes cleanly to 8-bit or 4-bit. QLoRA-style loading lets you stack 5-10 LoRAs on top of a 4-bit Flux base without blowing memory.
...
- [Zhang, Rao, Agrawala (2023). Adding Conditional Control to Text-to-Image 扩散模型 Models](https://arxiv.org/abs/2302.05543) — ControlNet.
- [Hu et al. (2021). LoRA: 低秩适配 of Large Language Models](https://arxiv.org/abs/2106.09685) — LoRA (originally for LLMs; ports to diffusion).
...
- [Ruiz et al. (2023). DreamBooth: Fine Tuning Text-to-Image 扩散模型 Models for Subject-Driven Generation](https://arxiv.org/abs/2208.12242) — DreamBooth.
