...
- **Codec quality caps output quality.** 如果 the codec can't represent a sound faithfully, no amount of generator quality helps. DAC is the current open best.
- **RVQ error accumulation.** Each RVQ layer models the residual of the previous. Errors on layer 1 propagate. 采样 with temperature 0 on higher layers helps.
...
- **Clean-data appetite.** Music generators need tens of thousands of 小时 of licensed music. The Suno / Udio RIAA lawsuit (2024) brought this to the surface.
...
|------|-----------------|-----------------------|
| Codec | "Neural compression" | 编码器 / decoder for audio; typical output is 50-75 Hz tokens. |
...
Audio is the one output modality users expect to arrive *as it is generated*, not all-at-once. In production terms this means TPOT matters (Time Per Output Token) because the user's listening speed is the target throughput — not their reading speed. 对于 16kHz audio tokenized at ~75 tokens/second (Encodec), the server must generate ≥75 tokens/sec per user to keep playback smooth.
...
如果 the product is "live voice chat" or "real-time music continuation", pick the codec AR path. 如果 it is "render a 30-second clip on submit", flow-matching wins on quality and total latency.
...
- [Liu et al. (2023). AudioLDM 2: Learning Holistic 音频生成 with Self-supervised Pretraining](https://arxiv.org/abs/2308.05734) — AudioLDM 2.
